...
;>>>> [video/M76V020] PUSHA at the entry of a video/blanking service routine: it saves all general registers, then sets DS=0x40 (kernel data segment) and reads the M76V020 control shadow [0x7b9], clears bit 0x40 (and al,0xbf) and writes it to video control port 0x343 to update display mode, then loads the active screen-state word [0x7ba] into [0x978].  // PUSHA (60) at routine start; following 'mov ax,0x40'/'mov ds,ax', 'mov al,[0x7b9]'/'mov dx,0x343'/'out dx,al'
00006010  60                pusha
00006011  1E                push ds
00006012  06                push es
...
0000616A  E101              loope loc_0616D   ; ->0x616D
0000616C  CB                retf
loc_0616D:
...
0000617D  06                push es
;>>>> [string/data] Data misdecoded as code: this 0xFA ('cli') sits inside a little-endian word/offset table in the early firmware data region; it is bracketed by bogus add/sbb decodes, a retf (0x616C), and 0x01CB filler words (0x618D-0x6195), confirming it is table/pointer data rather than an executed CLI.  // FA between push es and 'add sp,sp / add ax,0x4fa', followed by retf and repeated 01CB filler
0000617E  FA                cli
...
0000619C  740F              jz loc_061AD   ; ->0x61AD
0000619E  3C07              cmp al,0x7
000061A0  740B              jz loc_061AD   ; ->0x61AD
000061A2  3C00              cmp al,0x0
000061A4  7407              jz loc_061AD   ; ->0x61AD
...
000061C5  7512              jnz loc_061D9   ; ->0x61D9
...
;>>>> [INT21-DOS] Bounds-checks the capture/log buffer write index DI=[0x1ce] against limit 0x173 in a char-output routine: if below limit it appends AL to [di] and bumps the index, before also echoing the char to VRAM segment 0xC000 via the [0x1d0] 16-byte wrap pointer.  // mov di,[0x1ce] / cmp di,0x173 / jnc 0x61fa / mov [di],al / mov di,0xc000 / mov es,di / stosb
000061ED  81FF7301          cmp di,0x173
000061F1  7307              jnc dos_vram_061FA   ; ->0x61FA
...
00006270  E81300            call sub_06286   ; ->0x6286
00006273  3BEA              cmp bp,dx
00006275  7402              jz loc_06279   ; ->0x6279
...
00006292  E2FB              loop loc_0628F   ; ->0x628F
00006294  1F                pop ds
00006295  C3                ret
loc_06296:
...
loc_062B9:
000062B9  E8F811            call sub_074B4   ; ->0x74B4
loc_062BC:
000062BC  1E                push ds
000062BD  07                pop es
...
000062E0  C3                ret
loc_062E1:
000062E1  E8F23B            call sub_09ED6   ; ->0x9ED6
sub_062E4:
000062E4  C3                ret
...
000062F1  750F              jnz loc_06302   ; ->0x6302
000062F3  80FC32            cmp ah,0x32
000062F6  74E9              jz loc_062E1   ; ->0x62E1
000062F8  80FCAD            cmp ah,0xad
000062FB  74E4              jz loc_062E1   ; ->0x62E1
...
00006323  748F              jz loc_062B4   ; ->0x62B4
00006325  3C08              cmp al,0x8
00006327  7490              jz loc_062B9   ; ->0x62B9
...
00006342  E87000            call dos_fread_063B5   ; ->0x63B5
00006345  3C05              cmp al,0x5
00006347  7411              jz loc_0635A   ; ->0x635A
00006349  3C08              cmp al,0x8
0000634B  740D              jz loc_0635A   ; ->0x635A
0000634D  3C12              cmp al,0x12
0000634F  7409              jz loc_0635A   ; ->0x635A
00006351  3C1A              cmp al,0x1a
00006353  7444              jz loc_06399   ; ->0x6399
...
loc_06369:
00006369  E84900            call dos_fread_063B5   ; ->0x63B5
0000636C  3C20              cmp al,0x20
0000636E  720D              jc loc_0637D   ; ->0x637D
...
00006385  E864FF            call sub_062EC   ; ->0x62EC
00006388  E81C00            call dos_fread_063A7   ; ->0x63A7
;>>>> [INT21-DOS] Restores SI (saved at 0x6381) after the call to the [0x874]-flag subroutine at 0x63a7, in a character-fetch loop that reads bytes via call 0x63b5 and skips LF(0x0A)/CR(0x0D) before reprocessing.  // 0x638B 5E 'pop si' matches push si at 0x6381; followed by call 0x63b5 then cmp al,0x0a/0x0d
0000638B  5E                pop si
loc_0638C:
0000638C  E82600            call dos_fread_063B5   ; ->0x63B5
0000638F  3C0A              cmp al,0xa
00006391  74F9              jz loc_0638C   ; ->0x638C
00006393  3C0D              cmp al,0xd
00006395  74F5              jz loc_0638C   ; ->0x638C
...
000063B2  E978FF            jmp loc_0632D   ; ->0x632D
dos_fread_063B5:
000063B5  81FE4A0A          cmp si,0xa4a
000063B9  7302              jnc loc_063BD   ; ->0x63BD
000063BB  AC                lodsb
000063BC  C3                ret
loc_063BD:
...
000063DC  7406              jz loc_063E4   ; ->0x63E4
000063DE  1E                push ds
000063DF  07                pop es
...
000063F7  72F3              jc loc_063EC   ; ->0x63EC
000063F9  C3                ret
...
00006470  7503              jnz loc_06475   ; ->0x6475
...
00006486  780A              js dos_print_status_06492   ; ->0x6492
00006488  BA3702            mov dx,0x237
0000648B  3C02              cmp al,0x2
0000648D  7403              jz dos_print_status_06492   ; ->0x6492
...
000064BA  7203              jc loc_064BF   ; ->0x64BF
...
000064E9  75ED              jnz loc_064D8   ; ->0x64D8
000064EB  AC                lodsb
;>>>> [printer] String-output loop to the parallel printer: after LODSB fetches the next byte it is compared to '$' (0x24) as the terminator; non-'$' chars are sent via INT 17h AH=0 (print character, DX=0) at 0x64f4 after first polling printer status (INT 17h AH=2) at 0x64dc. Part of the MFGTEST printer-port string output.  // lodsb then cmp al,0x24 ('$'), with int 0x17 (printer) calls and xor ah,ah/xor dx,dx surrounding
...
00006507  7503              jnz loc_0650C   ; ->0x650C
...
00006528  75E2              jnz loc_0650C   ; ->0x650C
...
0000653D  7202              jc loc_06541   ; ->0x6541
0000653F  32C0              xor al,al
loc_06541:
00006541  A2AA07            mov [0x7aa],al
00006544  50                push ax
00006545  E86444            call sub_0A9AC   ; ->0xA9AC
00006548  58                pop ax
00006549  1E                push ds
0000654A  0E                push cs
0000654B  1F                pop ds
...
000065BA  7205              jc loc_065C1   ; ->0x65C1
...
000065ED  7205              jc loc_065F4   ; ->0x65F4
...
00006632  7403              jz loc_06637   ; ->0x6637
...
00006656  742C              jz loc_06684   ; ->0x6684
video_06658:
...
00006683  C3                ret
loc_06684:
00006684  E90FFC            jmp loc_06296   ; ->0x6296
00006687  50                push ax
...
000066A7  7202              jc loc_066AB   ; ->0x66AB
...
00006721  E85C00            call sub_06780   ; ->0x6780
00006724  5E                pop si
00006725  83C608            add si,byte +0x8
00006728  81FEF108          cmp si,0x8f1
0000672C  72F0              jc loc_0671E   ; ->0x671E
...
0000676F  E8B904            call sub_06C2B   ; ->0x6C2B
00006772  59                pop cx
...
00006789  7433              jz loc_067BE   ; ->0x67BE
...
000067C5  72BD              jc loc_06784   ; ->0x6784
000067C7  C3                ret
sub_067C8:
...
000067EF  74F9              jz loc_067EA   ; ->0x67EA
000067F1  A820              test al,0x20
000067F3  7408              jz loc_067FD   ; ->0x67FD
...
loc_06824:
00006824  E91D01            jmp loc_06944   ; ->0x6944
loc_06827:
00006827  C606E00600        mov byte [0x6e0],0x0
;>>>> [selftest] Zeroes the 16-bit counter/state word [0x76f] before the self-test reads the video status port (in al,0x340 just above) and prints message 0x56a, then walks the 8-byte flash/memory block descriptors at [0x8b1]. Initializes a byte-count or error accumulator for the MFGTEST memory/flash sizing routine.  // C7066F070000 mov word [0x76f],0x0 immediately before mov dx,0x56a / AH=9 INT 21h and the [0x8b1] descriptor loop
...
0000684F  7611              jna loc_06862   ; ->0x6862
00006851  50                push ax
00006852  51                push cx
00006853  57                push di
;>>>> [INT21-DOS] Sets CX=DI (the remaining bytes to next 0x1000/4K paragraph boundary, computed as 0x1000-(dx&0xFFF)) as the byte count for the block-clear/copy call to 0x6949; part of the allocator/initializer loop walking 8-byte descriptors from [bx] up to 0x8F1, filling memory in 4K-aligned chunks.  // 8BCF mov cx,di after sub di,si / cmp cx,di; E8F000 call 0x6949; loop bx+=8 until 0x8f1
00006854  8BCF              mov cx,di
00006856  E8F000            call dos_blk_copy_06949   ; ->0x6949
00006859  5F                pop di
0000685A  59                pop cx
0000685B  58                pop ax
...
loc_06862:
00006862  E8E400            call dos_blk_copy_06949   ; ->0x6949
00006865  83C308            add bx,byte +0x8
00006868  81FBF108          cmp bx,0x8f1
0000686C  72D0              jc loc_0683E   ; ->0x683E
...
00006885  759D              jnz loc_06824   ; ->0x6824
00006887  1E                push ds
00006888  07                pop es
...
000068A7  7410              jz loc_068B9   ; ->0x68B9
000068A9  AA                stosb
000068AA  81FF8207          cmp di,0x782
000068AE  72E3              jc loc_06893   ; ->0x6893
loc_068B0:
000068B0  81FF7307          cmp di,0x773
000068B4  76DD              jna loc_06893   ; ->0x6893
000068B6  4F                dec di
...
000068C0  72F9              jc loc_068BB   ; ->0x68BB
...
000068D8  E2FB              loop loc_068D5   ; ->0x68D5
000068DA  1F                pop ds
...
000068FA  7404              jz loc_06900   ; ->0x6900
000068FC  3C59              cmp al,0x59
000068FE  7506              jnz loc_06906   ; ->0x6906
loc_06900:
...
0000692F  E84002            call sub_06B72   ; ->0x6B72
00006932  B006              mov al,0x6
00006934  E83B02            call sub_06B72   ; ->0x6B72
00006937  B007              mov al,0x7
00006939  E83602            call sub_06B72   ; ->0x6B72
0000693C  8BC2              mov ax,dx
0000693E  E8CD02            call dos_hexprint_print_06C0E   ; ->0x6C0E
...
00006957  7402              jz loc_0695B   ; ->0x695B
00006959  0C01              or al,0x1
loc_0695B:
0000695B  F7C20020          test dx,0x2000
0000695F  7402              jz loc_06963   ; ->0x6963
...
000069CD  7410              jz loc_069DF   ; ->0x69DF
000069CF  33FF              xor di,di
000069D1  BE8307            mov si,0x783
000069D4  B91000            mov cx,0x10
000069D7  F3A6              repe cmpsb
000069D9  7401              jz loc_069DC   ; ->0x69DC
loc_069DB:
000069DB  C3                ret
loc_069DC:
000069DC  E97909            jmp loc_07358   ; ->0x7358
loc_069DF:
000069DF  1E                push ds
000069E0  1E                push ds
000069E1  06                push es
...
00006A78  7408              jz loc_06A82   ; ->0x6A82
...
00006A91  E834FD            call sub_067C8   ; ->0x67C8
...
00006AB8  7611              jna loc_06ACB   ; ->0x6ACB
00006ABA  50                push ax
00006ABB  51                push cx
00006ABC  57                push di
00006ABD  8BCF              mov cx,di
00006ABF  E86800            call sub_06B2A   ; ->0x6B2A
00006AC2  5F                pop di
00006AC3  59                pop cx
00006AC4  58                pop ax
...
loc_06ACB:
00006ACB  E85C00            call sub_06B2A   ; ->0x6B2A
00006ACE  83C308            add bx,byte +0x8
00006AD1  81FBF108          cmp bx,0x8f1
00006AD5  72D0              jc loc_06AA7   ; ->0x6AA7
00006AD7  803EE00600        cmp byte [0x6e0],0x0
00006ADC  7543              jnz loc_06B21   ; ->0x6B21
00006ADE  1E                push ds
00006ADF  33D2              xor dx,dx
00006AE1  BB0080            mov bx,0x8000
00006AE4  E8A100            call sub_06B88   ; ->0x6B88
00006AE7  BB0090            mov bx,0x9000
00006AEA  E89B00            call sub_06B88   ; ->0x6B88
00006AED  BB00A0            mov bx,0xa000
00006AF0  E89500            call sub_06B88   ; ->0x6B88
00006AF3  BB00B0            mov bx,0xb000
00006AF6  E88F00            call sub_06B88   ; ->0x6B88
00006AF9  1F                pop ds
00006AFA  89166107          mov [0x761],dx
00006AFE  E87335            call sub_0A074   ; ->0xA074
00006B01  E84C07            call sub_07250   ; ->0x7250
00006B04  721B              jc loc_06B21   ; ->0x6B21
00006B06  E8EC00            call sub_06BF5   ; ->0x6BF5
...
00006B34  7402              jz loc_06B38   ; ->0x6B38
00006B36  0C01              or al,0x1
loc_06B38:
00006B38  F7C20020          test dx,0x2000
00006B3C  7402              jz loc_06B40   ; ->0x6B40
;>>>> [video/M76V020] Sets bit 1 (or al,0x2) of the M76V020 mode/control byte being assembled from DX's 0x2000 flag, before merging with the RAM shadow [0x7B9] and writing to video control port 0x343; builds the column/mode control word for the graphics gate array.  // 0C02 or al,0x2 after test dx,0x2000; then mov dx,0x343 / and ah,[0x7b9] / out dx,al / mov [0x7b9],al
...
00006B65  E8FC39            call video_blk_fill_0A564   ; ->0xA564
00006B68  1F                pop ds
00006B69  7405              jz loc_06B70   ; ->0x6B70
...
00006B94  E2FB              loop loc_06B91   ; ->0x6B91
00006B96  1F                pop ds
00006B97  C3                ret
sub_06B98:
...
00006BFA  7411              jz loc_06C0D   ; ->0x6C0D
...
00006C0E  E80600            call sub_06C17   ; ->0x6C17
00006C11  E80300            call sub_06C17   ; ->0x6C17
sub_06C14:
00006C14  E80000            call sub_06C17   ; ->0x6C17
sub_06C17:
00006C17  C1C004            rol ax,byte 0x4
00006C1A  50                push ax
00006C1B  240F              and al,0xf
00006C1D  3C0A              cmp al,0xa
00006C1F  7202              jc loc_06C23   ; ->0x6C23
...
00006C34  7405              jz loc_06C3B   ; ->0x6C3B
00006C36  52                push dx
00006C37  E80300            call sub_06C3D   ; ->0x6C3D
00006C3A  5A                pop dx
loc_06C3B:
00006C3B  8BC2              mov ax,dx
sub_06C3D:
00006C3D  B9E803            mov cx,0x3e8
00006C40  E81000            call sub_06C53   ; ->0x6C53
00006C43  B96400            mov cx,0x64
00006C46  E80A00            call sub_06C53   ; ->0x6C53
00006C49  B90A00            mov cx,0xa
00006C4C  E80400            call sub_06C53   ; ->0x6C53
...
00006C59  7505              jnz loc_06C60   ; ->0x6C60
00006C5B  85C0              test ax,ax
00006C5D  7407              jz loc_06C66   ; ->0x6C66
00006C5F  45                inc bp
loc_06C60:
...
00006C68  C3                ret
sub_06C69:
00006C69  E80600            call sub_06C72   ; ->0x6C72
00006C6C  E80300            call sub_06C72   ; ->0x6C72
sub_06C6F:
00006C6F  E80000            call sub_06C72   ; ->0x6C72
sub_06C72:
00006C72  C1C004            rol ax,byte 0x4
00006C75  50                push ax
00006C76  240F              and al,0xf
00006C78  3C0A              cmp al,0xa
00006C7A  7202              jc loc_06C7E   ; ->0x6C7E
...
00006C8D  7501              jnz comm_06C90   ; ->0x6C90
00006C8F  C3                ret
comm_06C90:
...
00006CA2  E8C4FF            call sub_06C69   ; ->0x6C69
...
00006CAF  AA                stosb
00006CB0  AD                lodsw
00006CB1  E8B5FF            call sub_06C69   ; ->0x6C69
00006CB4  E2F7              loop loc_06CAD   ; ->0x6CAD
...
00006CC4  7204              jc loc_06CCA   ; ->0x6CCA
00006CC6  3C80              cmp al,0x80
00006CC8  7202              jc loc_06CCC   ; ->0x6CCC
loc_06CCA:
00006CCA  B02E              mov al,0x2e
loc_06CCC:
00006CCC  80FC20            cmp ah,0x20
00006CCF  7205              jc loc_06CD6   ; ->0x6CD6
00006CD1  80FC80            cmp ah,0x80
00006CD4  7202              jc loc_06CD8   ; ->0x6CD8
loc_06CD6:
00006CD6  B42E              mov ah,0x2e
loc_06CD8:
00006CD8  AB                stosw
00006CD9  E2E6              loop loc_06CC1   ; ->0x6CC1
00006CDB  B027              mov al,0x27
00006CDD  AA                stosb
00006CDE  4A                dec dx
00006CDF  75B4              jnz loc_06C95   ; ->0x6C95
...
00006CF8  E2FB              loop loc_06CF5   ; ->0x6CF5
00006CFA  8BC2              mov ax,dx
00006CFC  E86AFF            call sub_06C69   ; ->0x6C69
...
00006D19  7408              jz loc_06D23   ; ->0x6D23
00006D1B  3C2D              cmp al,0x2d
00006D1D  7508              jnz loc_06D27   ; ->0x6D27
00006D1F  81EE0002          sub si,0x200
loc_06D23:
00006D23  81C60001          add si,0x100
loc_06D27:
00006D27  E95BFF            jmp loc_06C85   ; ->0x6C85
loc_06D2A:
00006D2A  C3                ret
...
00006D37  0E                push cs
00006D38  07                pop es
;>>>> [string/data] This byte (0x0E) is part of the INT 4Fh character/control dispatch table at file 0x6D2C, not code. It is the low byte of the repeated default-handler word offset 0x0E07; the dispatch is performed by 'jmp [cs:bx+0xd2c]' at 0x6E02. ndisasm misdecodes table entries 0x0E,0x07 as 'push cs / pop es'.  // ROM dump at 0x6D2C shows repeated '0E 07' word entries; 'jmp [cs:bx+0xd2c]' at 0x6E02 indexes this table
00006D39  0E                push cs
00006D3A  07                pop es
00006D3B  0E                push cs
...
00006D4A  07                pop es
00006D4B  0E                push cs
;>>>> [string/data] Data misdecoded as code: this 0x07 byte is part of a jump/dispatch table (run of 0E 07 = push cs/pop es pairs interleaved with table words). It is one entry of the INT 4Fh/escape-handler vector table that the routine at 0x6DF4 indexes via JMP [cs:bx+0xd2c]. Not an executed instruction.  // long uniform run of 0E/07 byte pairs at 0x6D31-0x6D58, indexed by jmp [cs:bx+0xd2c] at 0x6E02
00006D4C  07                pop es
00006D4D  0E                push cs
00006D4E  07                pop es
00006D4F  0E                push cs
...
00006D53  0E                push cs
00006D54  07                pop es
;>>>> [string/data] Data, not code: this 0x0E byte is one half of the repeated default-handler word 0x0E07 filling the INT 4Fh character/control dispatch table at file 0x6D2C; ndisasm misdecodes the 0x0E/0x07 table words as 'push cs / pop es'. The dispatch is done via 'jmp [cs:bx+0x6d2c]' at 0x6E02.  // long run of alternating 0E (push cs) / 07 (pop es) = word 0x0E07 default entries
00006D55  0E                push cs
00006D56  07                pop es
00006D57  0E                push cs
00006D58  07                pop es
...
00006D7C  07                pop es
00006D7D  0E                push cs
;>>>> [string/data] Data, not code: low byte (0x07) of a word entry in the INT 4Fh dispatch table at 0x6D2C. The surrounding 0x0E/0x07 pairs are the default handler offset 0x0E07 misdecoded as 'pop es'.  // ROM 0x6D7C: '07 0E 07 0E B2 0E ...' little-endian word offsets into INT4F handler table
00006D7E  07                pop es
00006D7F  0E                push cs
...
00006DCA  FB                sti
00006DCB  43                inc bx
00006DCC  7F0E              jg loc_06DDC   ; ->0x6DDC
00006DCE  6C                insb
00006DCF  0E                push cs
00006DD0  07                pop es
00006DD1  0E                push cs
...
00006DEE  07                pop es
00006DEF  0E                push cs
;>>>> [string/data] Data byte (0x6C) misdecoded as 'insb'; it sits inside a run of 0x0E/0x07 filler/table bytes that is jump-table or padding data preceding the INT 4Fh-related handler at 0x6df5, not a real I/O-string instruction.  // single 0x6C amid long '0E 07' (push cs/pop es) run, no I/O port setup; not reached by real control flow
00006DF0  6C                insb
00006DF1  0E                push cs
00006DF2  FB                sti
00006DF3  FC                cld
00006DF4  50                push ax
;>>>> [INT40-comm] Saves BX at entry of a function-dispatch handler that builds a jump-table index from AH (bl=ah, bh=0, bx*=2), range-checks <0xC6, and does jmp [cs:bx+0xd2c]; this is the prologue PUSH BX before BX is clobbered as the index.  // 0x6DF5 53 'push bx' followed by mov bl,ah / xor bh,bh / add bx,bx / cmp bx,0xc6 / jmp [cs:bx+0xd2c]; later pop bx at 0x6E0F
00006DF5  53                push bx
...
;>>>> [INT40-comm] ISR epilogue: restores AX (POP AX) before IRET, completing the register teardown of an interrupt/kernel handler that loaded DS=0x0400, fetched [0x914] then [bx+0x4] (a pointer field of the active-page control block).  // pop ds; add sp,2; >>>>pop ax; iret directly after reading [0x914]->[bx+0x4]
00006E7D  58                pop ax
00006E7E  CF                iret
00006E7F  1E                push ds
00006E80  B84000            mov ax,0x40
;>>>> [INT40-comm] MOV DS,AX (AX=0x40) sets DS to the kernel data segment (physical 0x400) at the start of a control-block lookup: the routine then walks page/session control blocks on a 0xB8-byte stride from 0x18DE up to 0x245E, comparing the search key AX against each block's [bx+0x4] field (active flag at [bx+0xa7]) to locate the matching control block, returning STC on no match.  // 'mov ax,0x40'/'mov ds,ax'; following 'mov bx,0x18de'/'cmp byte [bx+0xa7],0x0'/'cmp ax,[bx+0x4]'/'add bx,0xb8'/'cmp bx,0x245e'
00006E83  8ED8              mov ds,ax
00006E85  8BC3              mov ax,bx
00006E87  BBDE18            mov bx,0x18de
loc_06E8A:
00006E8A  80BFA70000        cmp byte [bx+0xa7],0x0
00006E8F  7405              jz loc_06E96   ; ->0x6E96
00006E91  3B4704            cmp ax,[bx+0x4]
00006E94  7411              jz loc_06EA7   ; ->0x6EA7
loc_06E96:
00006E96  81C3B800          add bx,0xb8
00006E9A  81FB5E24          cmp bx,0x245e
00006E9E  72EA              jc loc_06E8A   ; ->0x6E8A
00006EA0  F9                stc
00006EA1  1F                pop ds
00006EA2  5B                pop bx
00006EA3  58                pop ax
...
;>>>> [INT40-comm] ISR/handler epilogue: restores the caller's DS (saved on entry) prior to the IRET at 0x6F10, after the routine has converted a binary value to a scaled/decimal result (mul 100, div 0x37). Part of register teardown POP DS/BX/AX then IRET.  // 0x6F0D 1F 'pop ds' immediately followed by pop bx (5B), pop ax (58), iret (CF) at 0x6F10
00006F0D  1F                pop ds
00006F0E  5B                pop bx
00006F0F  58                pop ax
00006F10  CF                iret
...
00006F8A  753B              jnz loc_06FC7   ; ->0x6FC7
...
00006FB9  750C              jnz loc_06FC7   ; ->0x6FC7
...
00007010  7406              jz loc_07018   ; ->0x7018
00007012  43                inc bx
00007013  8AFB              mov bh,bl
00007015  E901FF            jmp loc_06F19   ; ->0x6F19
loc_07018:
...
0000703B  7404              jz loc_07041   ; ->0x7041
...
0000705E  AB                stosw
0000705F  E2F7              loop loc_07058   ; ->0x7058
...
;>>>> [timer/ICU] STOSW stores the handler offset word 0x11DB into the IVT during interrupt-vector installation; DI=0xBC selects IVT entry 0x2F (0xBC/4). This is one of an unrolled series of MOV DI,off / MOV AX,offset / STOSW writes that pre-load the offset halves of several interrupt vectors (0x2F, then 0x5C at 0x170, 0x2A at 0xA8, 0x40 at 0x100, 0x21 at 0x84) before their CS segment words are written.  // preceding 'mov di,0xbc' / 'mov ax,0x11db'; STOSW (AB) to ES:DI with ES=0 (IVT); 0xBC/4 = INT 0x2F
00007071  AB                stosw
...
000070A6  E80736            call sub_0A6B0   ; ->0xA6B0
000070A9  E8C82F            call sub_0A074   ; ->0xA074
000070AC  E84D3E            call sub_0AEFC   ; ->0xAEFC
000070AF  E8FE04            call sub_075B0   ; ->0x75B0
000070B2  E80205            call sub_075B7   ; ->0x75B7
000070B5  E8EA2E            call sub_09FA2   ; ->0x9FA2
000070B8  FB                sti
...
000070C7  E8452D            call sub_09E0F   ; ->0x9E0F
...
000070DA  E8C8F8            call sub_069A5   ; ->0x69A5
000070DD  E8A638            call sub_0A986   ; ->0xA986
000070E0  E8FC41            call sub_0B2DF   ; ->0xB2DF
000070E3  E8134C            call sub_0BCF9   ; ->0xBCF9
000070E6  BAE307            mov dx,0x7e3
000070E9  E8B135            call sub_0A69D   ; ->0xA69D
000070EC  750B              jnz loc_070F9   ; ->0x70F9
000070EE  893E0008          mov [0x800],di
000070F2  8C060208          mov [0x802],es
000070F6  E8BE04            call sub_075B7   ; ->0x75B7
loc_070F9:
...
0000710A  750D              jnz dos_print_status_07119   ; ->0x7119
...
0000712C  E81F00            call selftest_fread_status_0714E   ; ->0x714E
0000712F  B92020            mov cx,0x2020
00007132  E81900            call selftest_fread_status_0714E   ; ->0x714E
;>>>> [selftest] Loads CX=0x8008 as the 4th test data pattern for the call to 0x714E, the M76V020 video gate-array register test loop (writes patterns to I/O 0x342, reads status from 0x340, accumulates errors into [0x7f0]/[0x7f1]/[0x7f2]); this is the last of four pattern calls (0x240,0x880,0x2020,0x8008) in the MFGTEST video-register verification.  // mov cx,0x8008 immediately followed by 'call 0x714e'; callee drives DX=0x342 OUT / DX=0x340 IN test loop
00007135  B90880            mov cx,0x8008
00007138  E81300            call selftest_fread_status_0714E   ; ->0x714E
0000713B  A0F107            mov al,[0x7f1]
0000713E  8A26F207          mov ah,[0x7f2]
00007142  F6D4              not ah
00007144  22E0              and ah,al
00007146  744D              jz loc_07195   ; ->0x7195
00007148  E85CF1            call sub_062A7   ; ->0x62A7
0000714B  E9C44A            jmp loc_0BC12   ; ->0xBC12
selftest_fread_status_0714E:
...
00007180  4F                dec di
00007181  75E4              jnz loc_07167   ; ->0x7167
...
00007194  C3                ret
loc_07195:
00007195  E817F2            call sub_063AF   ; ->0x63AF
00007198  E9774A            jmp loc_0BC12   ; ->0xBC12
0000719B  CA476E            retf 0x6e47
0000719E  4C                dec sp
0000719F  7B53              jpo loc_071F4   ; ->0x71F4
000071A1  1E                push ds
000071A2  44                inc sp
000071A3  4A                dec dx
000071A4  3F                aas
...
00007245  7303              jnc loc_0724A   ; ->0x724A
...
00007255  7411              jz loc_07268   ; ->0x7268
00007257  1E                push ds
00007258  FF36D507          push word [0x7d5]
0000725C  E88BFF            call sub_071EA   ; ->0x71EA
0000725F  7309              jnc loc_0726A   ; ->0x726A
...
0000727D  75E8              jnz loc_07267   ; ->0x7267
...
0000728E  7202              jc loc_07292   ; ->0x7292
...
0000729E  7202              jc loc_072A2   ; ->0x72A2
...
000072BB  7202              jc loc_072BF   ; ->0x72BF
000072BD  8BD1              mov dx,cx
loc_072BF:
000072BF  8CD8              mov ax,ds
000072C1  E84F00            call sub_07313   ; ->0x7313
000072C4  7416              jz loc_072DC   ; ->0x72DC
000072C6  8BC8              mov cx,ax
000072C8  8CC0              mov ax,es
000072CA  E84600            call sub_07313   ; ->0x7313
000072CD  740D              jz loc_072DC   ; ->0x72DC
000072CF  3BC1              cmp ax,cx
000072D1  7302              jnc loc_072D5   ; ->0x72D5
...
000072E0  7502              jnz loc_072E4   ; ->0x72E4
000072E2  03DD              add bx,bp
loc_072E4:
000072E4  3BDA              cmp bx,dx
000072E6  72D7              jc loc_072BF   ; ->0x72BF
...
00007322  741E              jz loc_07342   ; ->0x7342
00007324  53                push bx
...
;>>>> [INT40-comm] Bounds/threshold compare in a table walk: CX holds an entry's field [bx+0x1e] multiplied by 4 (two add cx,cx), compared against limit DX; jc 0x734e branches when below threshold. Selects/validates a table entry.  // 0x7327 mov cl,[bx+0x1e]; 0x732D/0x732F add cx,cx (x4); 0x7331 3BCA 'cmp cx,dx' then jc 0x734e
00007331  3BCA              cmp cx,dx
00007333  7219              jc comm_0734E   ; ->0x734E
00007335  8BC8              mov cx,ax
loc_07337:
00007337  3BF2              cmp si,dx
00007339  730C              jnc loc_07347   ; ->0x7347
0000733B  AD                lodsw
0000733C  85C0              test ax,ax
0000733E  74F7              jz loc_07337   ; ->0x7337
...
0000737A  7405              jz loc_07381   ; ->0x7381
...
00007390  7503              jnz loc_07395   ; ->0x7395
00007392  E80400            call sub_07399   ; ->0x7399
loc_07395:
00007395  1F                pop ds
00007396  07                pop es
00007397  61                popa
...
00007424  7405              jz loc_0742B   ; ->0x742B
00007426  50                push ax
00007427  E88235            call sub_0A9AC   ; ->0xA9AC
0000742A  58                pop ax
loc_0742B:
0000742B  A808              test al,0x8
0000742D  742B              jz loc_0745A   ; ->0x745A
0000742F  50                push ax
...
0000745C  7405              jz loc_07463   ; ->0x7463
0000745E  50                push ax
0000745F  E8CC2A            call 0x9f2e
00007462  58                pop ax
loc_07463:
00007463  A804              test al,0x4
00007465  7405              jz loc_0746C   ; ->0x746C
00007467  50                push ax
00007468  E84C01            call sub_075B7   ; ->0x75B7
0000746B  58                pop ax
loc_0746C:
0000746C  A810              test al,0x10
0000746E  7405              jz loc_07475   ; ->0x7475
00007470  50                push ax
00007471  E8DB3A            call sub_0AF4F   ; ->0xAF4F
00007474  58                pop ax
loc_07475:
00007475  C3                ret
sub_07476:
...
;>>>> [INT40-comm] Reads byte at offset 0x15 of the loaded module/exec structure (ES=[0x7d5] = the loaded executable's segment, SI=[es:0] = first record) and stores it to terminal state byte [0x7af]; extracts a config/mode field from the downloaded module header into the kernel's working flags.  // mov es,[0x7d5]; mov si,[es:si]; >>>> mov al,[es:si+0x15]; mov [0x7af],al ([0x7d5] is the loaded-segment pointer set at 0x7227)
...
00007498  7511              jnz loc_074AB   ; ->0x74AB
...
000074BE  7406              jz comm_074C6   ; ->0x74C6
;>>>> [INT40-comm] Pointer-advance (INC SI twice = +2 bytes per word entry) inside a 6-entry word-table linear search: scans the 6-word table at 0x7ba for AX (==0) and on no-match falls through to STC/RET (not found). The matched-entry index is stored at [0x7c6] on success. Part of a config/mode table lookup.  // mov si,0x7ba; mov cx,6; cmp ax,[si]; jz 0x74c6; inc si/inc si; loop 0x74bc; stc/ret; mov [0x7c6],si
000074C0  46                inc si
000074C1  46                inc si
000074C2  E2F8              loop comm_074BC   ; ->0x74BC
000074C4  F9                stc
000074C5  C3                ret
comm_074C6:
...
00007557  7225              jc loc_0757E   ; ->0x757E
00007559  7407              jz loc_07562   ; ->0x7562
...
;>>>> [INT40-comm] Ring-buffer empty test in an INT 40h AH=0 (serial/keyboard get-char) handler: after loading head pointer SI=[bx+0x90] and comparing to tail [bx+0x8e] of the input queue (control block at DS:[0x914]=segment 0x40), JZ skips the 'mov ax,[si]' peek when head==tail (buffer empty).  // mov si,[bx+0x90] / cmp si,[bx+0x8e] / jz 0x7572 / mov ax,[si]
0000756E  7402              jz comm_blk_fhandle_07572   ; ->0x7572
00007570  8B04              mov ax,[si]
comm_blk_fhandle_07572:
00007572  1F                pop ds
00007573  5E                pop si
00007574  5B                pop bx
...
0000758A  74EC              jz loc_07578   ; ->0x7578
0000758C  80FC40            cmp ah,0x40
0000758F  AD                lodsw
00007590  750A              jnz loc_0759C   ; ->0x759C
...
000075A2  7204              jc loc_075A8   ; ->0x75A8
...
;>>>> [video/M76V020] REP MOVSW copying 0xCE (206) words of font/screen-init data from source 0x2094 into display row buffer at DI=0x124a; one of four consecutive rep-movsw blocks filling row buffers 0xe4a/0x104a/0x124a/0x144a during display initialization (matches accepted 0x75f0).  // mov si,0x2094 / mov di,0x124a / mov cx,0xce / rep movsw
...
00007607  1E                push ds
00007608  E80200            call esc_dispatch_0760D   ; ->0x760D
0000760B  1F                pop ds
0000760C  C3                ret
esc_dispatch_0760D:
...
0000761C  740B              jz esc_dispatch_07629   ; ->0x7629
0000761E  3A04              cmp al,[si]
00007620  7307              jnc esc_dispatch_07629   ; ->0x7629
00007622  8B5001            mov dx,[bx+si+0x1]
00007625  85D2              test dx,dx
00007627  7510              jnz loc_07639   ; ->0x7639
esc_dispatch_07629:
00007629  0E                push cs
0000762A  1F                pop ds
0000762B  BE3022            mov si,0x2230
0000762E  3A04              cmp al,[si]
00007630  7359              jnc loc_0768B   ; ->0x768B
00007632  8B5001            mov dx,[bx+si+0x1]
00007635  85D2              test dx,dx
00007637  7452              jz loc_0768B   ; ->0x768B
loc_07639:
00007639  03F2              add si,dx
0000763B  8AE0              mov ah,al
0000763D  AC                lodsb
;>>>> [INT4F-dispatch] Tests the byte just fetched by LODSB (AL) for the sign bit in the escape/control-code dispatch table walk: JS at 0x7640 branches when the high bit (end-of-string/terminator marker) is set, otherwise the byte is compared to AH and recursively processed via call 0x7629/0x760d. Part of the EMULATOR escape-sequence matcher.  // lodsb; test al,al; js 0x7652; cmp al,ah; call 0x7629 (self) / call 0x760d; table base si=0x2230
0000763E  84C0              test al,al
00007640  7810              js esc_dispatch_07652   ; ->0x7652
00007642  1E                push ds
00007643  56                push si
00007644  3AC4              cmp al,ah
00007646  7505              jnz loc_0764D   ; ->0x764D
00007648  E8DEFF            call esc_dispatch_07629   ; ->0x7629
0000764B  EB03              jmp short 0x7650
loc_0764D:
0000764D  E8BDFF            call esc_dispatch_0760D   ; ->0x760D
00007650  5E                pop si
00007651  1F                pop ds
esc_dispatch_07652:
00007652  AC                lodsb
...
0000765B  7529              jnz loc_07686   ; ->0x7686
...
0000767E  A5                movsw
;>>>> [video/M76V020] Advances destination pointer DI by 510 (0x1FE) bytes between single-word (movsw) copies, striding across display/buffer rows while building a table at 0xa4a, matching the row-stride layout of the terminal's screen buffers.  // add di,0x1fe interleaved with repeated 'movsw' starting at di=0xa4a
...
00007689  75D6              jnz loc_07661   ; ->0x7661
loc_0768B:
0000768B  C3                ret
kbd_0768C:
...
000076B9  7505              jnz loc_076C0   ; ->0x76C0
...
000076DE  7407              jz loc_076E7   ; ->0x76E7
000076E0  8BC2              mov ax,dx
000076E2  E94401            jmp loc_07829   ; ->0x7829
...
00007702  E83002            call esc_dispatch_clear_07935   ; ->0x7935
00007705  EBAE              jmp short 0x76b5
00007707  8026F607FE        and byte [0x7f6],0xfe
;>>>> [INT4F-dispatch] Branch in a control-code action dispatcher that clears mode/attribute flag bits in the terminal state byte [0x7f6]: JMP short skips one AND-mask case (0x7f6 &= 0xFB) to reach the test of [0x7f6]&0x05 at 0x7713 that decides whether to invoke the screen/mode-update routine call 0x7935. Implements terminal mode-reset (e.g. ANSI/escape RM) handling.  // and byte [0x7f6],0xfe; jmp 0x7713; test byte [0x7f6],5; jnz; and byte [0x7f5],0xfb; call 0x7935 (mode update); chain of and [0x7f6],mask cases
0000770C  EB05              jmp short 0x7713
0000770E  8026F607FB        and byte [0x7f6],0xfb
00007713  F606F60705        test byte [0x7f6],0x5
00007718  759B              jnz loc_076B5   ; ->0x76B5
0000771A  8026F507FB        and byte [0x7f5],0xfb
0000771F  E81302            call esc_dispatch_clear_07935   ; ->0x7935
loc_07722:
...
00007735  75EB              jnz loc_07722   ; ->0x7722
00007737  8026F507F7        and byte [0x7f5],0xf7
0000773C  E8F601            call esc_dispatch_clear_07935   ; ->0x7935
0000773F  E973FF            jmp loc_076B5   ; ->0x76B5
...
000077AA  7513              jnz loc_077BF   ; ->0x77BF
000077AC  0806F607          or [0x7f6],al
000077B0  F606F50704        test byte [0x7f5],0x4
000077B5  7508              jnz loc_077BF   ; ->0x77BF
...
000077C8  754F              jnz loc_07819   ; ->0x7819
000077CA  A0F507            mov al,[0x7f5]
000077CD  E80201            call sub_078D2   ; ->0x78D2
000077D0  7247              jc loc_07819   ; ->0x7819
000077D2  E88D00            call sub_07862   ; ->0x7862
...
000077FC  751C              jnz loc_0781A   ; ->0x781A
000077FE  80FEFF            cmp dh,0xff
00007801  7517              jnz loc_0781A   ; ->0x781A
00007803  80FA06            cmp dl,0x6
00007806  7312              jnc loc_0781A   ; ->0x781A
...
00007814  7403              jz loc_07819   ; ->0x7819
...
00007821  7433              jz loc_07856   ; ->0x7856
00007823  84F6              test dh,dh
00007825  7502              jnz loc_07829   ; ->0x7829
...
00007839  7204              jc comm_fhandle_0783F   ; ->0x783F
0000783B  8B9D8A00          mov bx,[di+0x8a]
comm_fhandle_0783F:
0000783F  3B9D9000          cmp bx,[di+0x90]
00007843  7412              jz comm_fhandle_07857   ; ->0x7857
...
00007864  7535              jnz loc_0789B   ; ->0x789B
00007866  A808              test al,0x8
00007868  7542              jnz loc_078AC   ; ->0x78AC
0000786A  A803              test al,0x3
0000786C  7539              jnz loc_078A7   ; ->0x78A7
0000786E  80FD47            cmp ch,0x47
00007871  720B              jc loc_0787E   ; ->0x787E
00007873  80FD53            cmp ch,0x53
00007876  7706              ja loc_0787E   ; ->0x787E
00007878  A820              test al,0x20
0000787A  751A              jnz loc_07896   ; ->0x7896
0000787C  EB3A              jmp short 0x78b8
loc_0787E:
0000787E  8B1EFE07          mov bx,[0x7fe]
00007882  80BF960000        cmp byte [bx+0x96],0x0
00007887  782F              js loc_078B8   ; ->0x78B8
00007889  7504              jnz loc_0788F   ; ->0x788F
0000788B  A840              test al,0x40
0000788D  7429              jz loc_078B8   ; ->0x78B8
loc_0788F:
0000788F  803EAB0700        cmp byte [0x7ab],0x0
00007894  7511              jnz loc_078A7   ; ->0x78A7
loc_07896:
...
000078A0  7419              jz loc_078BB   ; ->0x78BB
...
000078B1  7408              jz loc_078BB   ; ->0x78BB
...
;>>>> [video/M76V020] Reads a 16-bit value from the selected display row-buffer: BX was loaded with one of the row-buffer bases (0xa4a/0xe4a/0x124a/0x144a) chosen by AL bits, then BX += CH*2 as a word index; 'mov dx,[bx]' fetches that row/cell word and returns it in DX (companion to the 0xe4a/0x104a/0x124a/0x144a row buffers filled at 0x75xx).  // preceding mov bx,0x{a4a,e4a,124a,144a}; mov al,ch; xor ah,ah; add ax,ax; add bx,ax then mov dx,[bx]; ret
...
000078D4  7426              jz loc_078FC   ; ->0x78FC
000078D6  A808              test al,0x8
000078D8  7507              jnz loc_078E1   ; ->0x78E1
000078DA  F606F60740        test byte [0x7f6],0x40
000078DF  741B              jz loc_078FC   ; ->0x78FC
loc_078E1:
000078E1  80FD47            cmp ch,0x47
000078E4  720F              jc loc_078F5   ; ->0x78F5
000078E6  80FD53            cmp ch,0x53
000078E9  730A              jnc loc_078F5   ; ->0x78F5
000078EB  80FD4A            cmp ch,0x4a
000078EE  7405              jz loc_078F5   ; ->0x78F5
000078F0  80FD4E            cmp ch,0x4e
000078F3  7509              jnz loc_078FE   ; ->0x78FE
loc_078F5:
000078F5  803EFD0700        cmp byte [0x7fd],0x0
000078FA  7528              jnz loc_07924   ; ->0x7924
loc_078FC:
000078FC  F8                clc
000078FD  C3                ret
loc_078FE:
...
00007959  E806FF            call sub_07862   ; ->0x7862
0000795C  59                pop cx
0000795D  58                pop ax
0000795E  1F                pop ds
0000795F  5B                pop bx
...
00007970  740C              jz loc_0797E   ; ->0x797E
00007972  FA                cli
...
;>>>> [keyboard] INT character-available query: loads kernel ptr [0x914] into BX, checks pending flag [bx+0x98] and head [bx+0x90] vs tail [bx+0x8e]; returns AL=0x00 when buffer is empty (head==tail and flag clear) and AL=0xFF when a character is available. The AL=0xFF meaning in the claim is backwards.  // mov bx,[0x914] after push ds/mov bx,0x40/mov ds,bx; then cmp byte [bx+0x98],0 and cmp [bx+0x90],[bx+0x8e]; mov al,0xff/iret on empty
000079CD  8B1E1409          mov bx,[0x914]
000079D1  80BF980000        cmp byte [bx+0x98],0x0
000079D6  750A              jnz loc_079E2   ; ->0x79E2
000079D8  8B879000          mov ax,[bx+0x90]
000079DC  3B878E00          cmp ax,[bx+0x8e]
000079E0  7406              jz loc_079E8   ; ->0x79E8
loc_079E2:
000079E2  1F                pop ds
000079E3  5B                pop bx
000079E4  58                pop ax
...
000079FE  740B              jz loc_07A0B   ; ->0x7A0B
00007A00  C687980000        mov byte [bx+0x98],0x0
00007A05  1F                pop ds
;>>>> [INT16-keyboard] ISR epilogue: restores the caller's BX (saved on entry) before 'add sp,2' (discards saved AX/flags slot) and IRET. Tail of a keyboard/comm interrupt handler that had set DS=0x0400, tested the per-page flag [bx+0x98] via [0x914], and cleared it.  // pop ds; >>>>pop bx; add sp,byte +0x2; iret following the [bx+0x98] flag clear and the INT 16h path at 0x7a0e
00007A06  5B                pop bx
...
00007C79  E000              loopne loc_07C7B   ; ->0x7C7B
loc_07C7B:
00007C7B  37                aaa
...
00009941  7401              jz loc_09944   ; ->0x9944
00009943  48                dec ax
loc_09944:
...
0000995C  52                push dx
0000995D  50                push ax
0000995E  E8CA01            call sub_09B2B   ; ->0x9B2B
00009961  7243              jc loc_099A6   ; ->0x99A6
00009963  58                pop ax
00009964  5A                pop dx
00009965  52                push dx
00009966  50                push ax
...
;>>>> [INT21-DOS] First SHR DX,1 of a 4-bit logical right shift of the 32-bit value (size+0xF) in DX:AX, computing a paragraph (16-byte) count via four SHR DX/RCR AX pairs; part of the INT 21h memory allocator sizing a block before adding base [0x820].  // D1EA shr dx,1 begins sequence of D1EA/D1D8 pairs after add ax,0xf / adc dx,0; then add ax,[0x820]
0000996D  D1EA              shr dx,1
0000996F  D1D8              rcr ax,1
;>>>> [INT21-DOS/memory-alloc] One step of a 32-bit shift-right-by-4 (>>4) to convert a byte length (DX:AX, already +0xF) into a paragraph (16-byte) count, via repeated 'shr dx,1 / rcr ax,1'; feeds the heap allocator's size-in-paragraphs (later added to [0x820]).  // add ax,0xf / adc dx,0 / shr dx,1 / rcr ax,1 (x4) / add ax,[0x820]
00009971  D1EA              shr dx,1
00009973  D1D8              rcr ax,1
00009975  D1EA              shr dx,1
00009977  D1D8              rcr ax,1
00009979  D1EA              shr dx,1
0000997B  7526              jnz loc_099A3   ; ->0x99A3
0000997D  D1D8              rcr ax,1
0000997F  03062008          add ax,[0x820]
00009983  40                inc ax
00009984  803E720800        cmp byte [0x872],0x0
00009989  7403              jz loc_0998E   ; ->0x998E
0000998B  50                push ax
...
00009995  E8F801            call sub_09B90   ; ->0x9B90
00009998  A30A08            mov [0x80a],ax
0000999B  E8FA01            call sub_09B98   ; ->0x9B98
0000999E  5B                pop bx
0000999F  3BC3              cmp ax,bx
000099A1  7309              jnc loc_099AC   ; ->0x99AC
loc_099A3:
000099A3  B80800            mov ax,0x8
loc_099A6:
000099A6  83C404            add sp,byte +0x4
loc_099A9:
000099A9  E96201            jmp loc_09B0E   ; ->0x9B0E
loc_099AC:
000099AC  8B0E2208          mov cx,[0x822]
000099B0  2B0E2008          sub cx,[0x820]
000099B4  720A              jc dos_blk_099C0   ; ->0x99C0
000099B6  03D9              add bx,cx
;>>>> [INT21-DOS] Overflow guard in a heap/arena size-clamp: after add bx,cx (block-end = base+available, where cx=[0x822]-[0x820] is the free-region size), JC branches to 0x99c0 on 16-bit carry/overflow to clamp the requested allocation size AX to the available BX. Part of the INT 21h AH=48-style memory allocator computing the granted block size into [0x80e].  // mov cx,[0x822]; sub cx,[0x820]; add bx,cx; jc 0x99c0; cmp ax,bx; jna; mov ax,bx; mov [0x80e],ax
000099B8  7206              jc dos_blk_099C0   ; ->0x99C0
000099BA  3BC3              cmp ax,bx
000099BC  7602              jna dos_blk_099C0   ; ->0x99C0
000099BE  8BC3              mov ax,bx
dos_blk_099C0:
000099C0  A30E08            mov [0x80e],ax
000099C3  E8CA01            call sub_09B90   ; ->0x9B90
000099C6  72DB              jc loc_099A3   ; ->0x99A3
000099C8  A30C08            mov [0x80c],ax
000099CB  803E720800        cmp byte [0x872],0x0
000099D0  750A              jnz loc_099DC   ; ->0x99DC
...
000099ED  E84901            call sub_09B39   ; ->0x9B39
000099F0  72B7              jc loc_099A9   ; ->0x99A9
000099F2  8B0E1C08          mov cx,[0x81c]
000099F6  E339              jcxz loc_09A31   ; ->0x9A31
000099F8  8B0E2E08          mov cx,[0x82e]
000099FC  E82C01            call sub_09B2B   ; ->0x9B2B
000099FF  72A8              jc loc_099A9   ; ->0x99A9
00009A01  8B0E1C08          mov cx,[0x81c]
00009A05  33D2              xor dx,dx
loc_09A07:
00009A07  4A                dec dx
00009A08  7915              jns loc_09A1F   ; ->0x9A1F
00009A0A  1E                push ds
00009A0B  07                pop es
00009A0C  BF3208            mov di,0x832
00009A0F  51                push cx
00009A10  B94000            mov cx,0x40
00009A13  E85A01            call sub_09B70   ; ->0x9B70
00009A16  59                pop cx
00009A17  7290              jc loc_099A9   ; ->0x99A9
...
00009A2F  E2D6              loop loc_09A07   ; ->0x9A07
loc_09A31:
...
00009ADE  E8C50C            call sub_0A7A6   ; ->0xA7A6
...
00009B11  50                push ax
00009B12  E87E0C            call sub_0A793   ; ->0xA793
00009B15  F9                stc
...
00009B1B  07                pop es
00009B1C  5F                pop di
;>>>> [INT40-comm] Function epilogue: POP SI restores caller's SI as part of restoring DI/SI/DX/CX/BX before RETF 2; this is the far-call return sequence of a kernel API handler (a 9Axxxx:F600 callee) that returns status in carry (the stc/clc at 0x9B15/9B18).  // 5E pop si in the run 5F/5E/5A/59/5B followed by CA0200 retf 0x2; carry set/clear at 9B15/9B18
00009B1D  5E                pop si
00009B1E  5A                pop dx
00009B1F  59                pop cx
00009B20  5B                pop bx
...
00009B3F  742F              jz sub_09B70   ; ->0x9B70
loc_09B41:
00009B41  51                push cx
00009B42  52                push dx
00009B43  1E                push ds
...
00009B6C  59                pop cx
00009B6D  4A                dec dx
00009B6E  75D1              jnz loc_09B41   ; ->0x9B41
sub_09B70:
00009B70  E311              jcxz loc_09B83   ; ->0x9B83
...
;>>>> [INT40-comm] Sets DS=0x0040 (segment 0x40, physical 0x400 — the kernel data segment), so mov [0x874],al stores AL to physical 0xCB4 (0040:0874); records a kernel flag in an INT handler after AH=48h alloc, then pop ds/pop bx/iret. Flag is read at 0x63A7, not 0x638b.  // mov bx,0x40; mov ds,bx; mov [0x874],al; pop ds; pop bx; iret — [0x874] is the kernel flag used elsewhere (e.g. 0x638b)
...
00009BB4  7416              jz loc_09BCC   ; ->0x9BCC
...
00009BE5  7503              jnz loc_09BEA   ; ->0x9BEA
...
00009BF4  7438              jz loc_09C2E   ; ->0x9C2E
loc_09BF6:
00009BF6  8B5C02            mov bx,[si+0x2]
00009BF9  83FB00            cmp bx,byte +0x0
00009BFC  75F0              jnz loc_09BEE   ; ->0x9BEE
...
00009C91  F8                clc
;>>>> [INT40-comm] Far-return-with-success epilogue of an INT 40h/kernel block-handler: after popping di/si/dx/cx/ds/es/bx and 'add sp,2' (discarding the saved flags slot) the preceding CLC sets carry=0 (success) and 'retf 2' returns to the INT caller while discarding 2 stack bytes. Marks the OK exit of the memory/control-block routine whose next entry begins at 0x9c95.  // opcode CA0200 (retf 2) preceded by F8 (clc) and the di/si/dx/cx/ds/es/bx pop chain + 'add sp,2'
...
00009CD6  7620              jna loc_09CF8   ; ->0x9CF8
...
00009D01  7631              jna loc_09D34   ; ->0x9D34
...
00009D47  7510              jnz loc_09D59   ; ->0x9D59
...
loc_09D59:
00009D59  E8050C            call blk_0A961   ; ->0xA961
;>>>> [INT21-DOS/memory-alloc] Decrements AX to the previous paragraph after call 0xa961 returns a segment, so DS points at the block header that is then checked for signature 'CL' (0x434c, CONFIG.BLK/heap block magic).  // dec ax ; mov ds,ax ; cmp word [si],0x434c ('CL')
00009D5C  48                dec ax
00009D5D  8ED8              mov ds,ax
00009D5F  33F6              xor si,si
00009D61  B80700            mov ax,0x7
00009D64  813C4C43          cmp word [si],0x434c
00009D68  75E2              jnz loc_09D4C   ; ->0x9D4C
...
;>>>> [INT40-comm] Zeroes the word at [si] (the link/forward-pointer or count field at offset 0 of a memory-block/list node) while freeing/resetting the node; part of a linked-list memory manager release routine that also clears the offset-8 word and the offset-0xa flag byte before popping registers and RETF.  // C7040000 mov word [si],0x0 adjacent to C744080000 mov word [si+0x8],0x0 and C6440A00 mov byte [si+0xa],0x0
...
;>>>> [INT40-comm] Computes the size/end of a memory block by decrementing BX (BX = base+DI then -1 gives the last/end paragraph), used to initialize the second node of a two-node free-list split; the value is then stored into the node's offset-2 link field [es:si+2] and used as a new segment (mov es,bx).  // 83EB01 sub bx,0x1 right after 03DF add bx,di; result written via 26895C02 mov [es:si+0x2],bx and 8EC3 mov es,bx
...
00009E90  7506              jnz loc_09E98   ; ->0x9E98
00009E92  263B5C08          cmp bx,[es:si+0x8]
00009E96  7409              jz loc_09EA1   ; ->0x9EA1
loc_09E98:
00009E98  268B4402          mov ax,[es:si+0x2]
00009E9C  85C0              test ax,ax
00009E9E  75E9              jnz loc_09E89   ; ->0x9E89
00009EA0  C3                ret
loc_09EA1:
00009EA1  53                push bx
00009EA2  40                inc ax
...
00009EBF  750A              jnz comm_blk_09ECB   ; ->0x9ECB
00009EC1  263B5C08          cmp bx,[es:si+0x8]
;>>>> [INT40-comm] Linked-list traversal of kernel control blocks in segment [0x876]: if the in-use flag [es:si+0xa]==1 and the key BX==[es:si+8] do not BOTH match, skip the accumulate of size field [es:si+6] into AX; this branch is the 'key mismatch' skip.  // 0x9EBA cmp byte [es:si+0xa],1 / 0x9EC1 cmp bx,[es:si+8] then 0x9EC5 7504 'jnz 0x9ecb' bypassing add ax,[es:si+6]
00009EC5  7504              jnz comm_blk_09ECB   ; ->0x9ECB
00009EC7  26034406          add ax,[es:si+0x6]
comm_blk_09ECB:
00009ECB  268B5402          mov dx,[es:si+0x2]
00009ECF  85D2              test dx,dx
00009ED1  75E5              jnz loc_09EB8   ; ->0x9EB8
00009ED3  5A                pop dx
00009ED4  5E                pop si
00009ED5  C3                ret
sub_09ED6:
...
00009F08  7413              jz loc_09F1D   ; ->0x9F1D
...
00009F37  E86307            call sub_0A69D   ; ->0xA69D
00009F3A  7405              jz loc_09F41   ; ->0x9F41
00009F3C  0E                push cs
00009F3D  07                pop es
...
00009F5C  7202              jc loc_09F60   ; ->0x9F60
...
00009F6A  E309              jcxz loc_09F75   ; ->0x9F75
loc_09F6C:
00009F6C  803C00            cmp byte [si],0x0
00009F6F  7404              jz loc_09F75   ; ->0x9F75
00009F71  A4                movsb
00009F72  40                inc ax
00009F73  E2F7              loop loc_09F6C   ; ->0x9F6C
loc_09F75:
00009F75  59                pop cx
00009F76  5B                pop bx
00009F77  5E                pop si
...
00009FB6  7743              ja printer_print_dispatch_09FFB   ; ->0x9FFB
;>>>> [printer] Sub-function dispatch in an INT 17h/printer-or-video helper (DS set to kernel seg 0x40): after 'cmp ah,0x1', this 'jz 0xa001' takes the AH==1 path; the AH==0 fall-through reads video status port 0x340 (test bit 0x80 busy) and writes the char to port 0x342. Routes the printer/char-output function selector.  // cmp ah,0x1 / ja 0x9ffb / >>jz 0xa001; later in al,0x340 / test al,0x80 / out 0x342
00009FB8  7447              jz printer_print_dispatch_0A001   ; ->0xA001
00009FBA  85D2              test dx,dx
00009FBC  7549              jnz loc_0A007   ; ->0xA007
...
00009FF9  5B                pop bx
00009FFA  CF                iret
printer_print_dispatch_09FFB:
00009FFB  85D2              test dx,dx
00009FFD  751F              jnz loc_0A01E   ; ->0xA01E
00009FFF  EBE7              jmp short 0x9fe8
printer_print_dispatch_0A001:
...
0000A00E  7407              jz loc_0A017   ; ->0xA017
loc_0A010:
0000A010  803EA80702        cmp byte [0x7a8],0x2
0000A015  7509              jnz loc_0A020   ; ->0xA020
loc_0A017:
0000A017  9AFC4B00F6        call 0xf600:0x4bfc
0000A01C  72E5              jc loc_0A003   ; ->0xA003
loc_0A01E:
...
0000A029  7504              jnz loc_0A02F   ; ->0xA02F
;>>>> [INT40-comm] Tests bit 4 (0x10) of AL returned by the kernel call to F600:4B49; AL holds device/status flags. Bit 7 (0x80) was tested just before; this checks the 0x10 status bit and branches to set bh|=0x10 (vs bh|=0x28). Part of decoding a serial/device status byte into terminal attribute/state bits in BH.  // A810 test al,0x10 after 9A494B00F6 call 0xf600:0x4b49 and A880 test al,0x80; branches build bh with or bh,0x10/0x28/0x80
0000A02B  A810              test al,0x10
0000A02D  7505              jnz loc_0A034   ; ->0xA034
loc_0A02F:
...
0000A062  72F2              jc loc_0A056   ; ->0xA056
0000A064  FA                cli
...
0000A089  7413              jz loc_0A09E   ; ->0xA09E
0000A08B  BBA108            mov bx,0x8a1
0000A08E  3D8994            cmp ax,0x9489
0000A091  740B              jz loc_0A09E   ; ->0xA09E
loc_0A093:
...
0000A0B6  7408              jz loc_0A0C0   ; ->0xA0C0
0000A0B8  BEA108            mov si,0x8a1
0000A0BB  3D8994            cmp ax,0x9489
0000A0BE  75D3              jnz loc_0A093   ; ->0xA093
loc_0A0C0:
...
0000A0E9  E2E2              loop loc_0A0CD   ; ->0xA0CD
...
0000A0F8  7432              jz loc_0A12C   ; ->0xA12C
0000A0FA  FEC0              inc al
0000A0FC  742E              jz loc_0A12C   ; ->0xA12C
...
0000A162  1E                push ds
0000A163  06                push es
0000A164  E85100            call sub_0A1B8   ; ->0xA1B8
0000A167  E88E00            call sub_0A1F8   ; ->0xA1F8
0000A16A  7411              jz loc_0A17D   ; ->0xA17D
...
0000A185  740E              jz loc_0A195   ; ->0xA195
0000A187  83C30A            add bx,byte +0xa
0000A18A  81FBCA18          cmp bx,0x18ca
0000A18E  72F2              jc loc_0A182   ; ->0xA182
...
0000A1C8  74F5              jz loc_0A1BF   ; ->0xA1BF
0000A1CA  3C5C              cmp al,0x5c
0000A1CC  74F1              jz loc_0A1BF   ; ->0xA1BF
0000A1CE  3C20              cmp al,0x20
0000A1D0  7610              jna loc_0A1E2   ; ->0xA1E2
0000A1D2  E3F1              jcxz loc_0A1C5   ; ->0xA1C5
0000A1D4  3C61              cmp al,0x61
0000A1D6  7206              jc loc_0A1DE   ; ->0xA1DE
0000A1D8  3C7A              cmp al,0x7a
0000A1DA  7702              ja loc_0A1DE   ; ->0xA1DE
...
0000A1E8  7502              jnz loc_0A1EC   ; ->0xA1EC
0000A1EA  4F                dec di
0000A1EB  41                inc cx
loc_0A1EC:
0000A1EC  E304              jcxz dos_fill_0A1F2   ; ->0xA1F2
0000A1EE  B020              mov al,0x20
0000A1F0  F3AA              rep stosb
dos_fill_0A1F2:
0000A1F2  B84000            mov ax,0x40
0000A1F5  8ED8              mov ds,ax
;>>>> [INT21-DOS] Return from the filename/field space-padding routine; sets DS=0x40 (kernel data segment 0x0400) just before RET so the caller resumes with the kernel data segment after the rep-stosb space fill and trailing '.' handling.  // 0x0A1F2 mov ax,0x40 / mov ds,ax then 0x0A1F7 C3 'ret'; preceding rep stosb of 0x20 (space)
0000A1F7  C3                ret
sub_0A1F8:
0000A1F8  803EA00800        cmp byte [0x8a0],0x0
0000A1FD  7515              jnz loc_0A214   ; ->0xA214
0000A1FF  BEB108            mov si,0x8b1
loc_0A202:
0000A202  56                push si
0000A203  8B04              mov ax,[si]
0000A205  E81300            call sub_0A21B   ; ->0xA21B
0000A208  5E                pop si
0000A209  740F              jz loc_0A21A   ; ->0xA21A
0000A20B  83C608            add si,byte +0x8
0000A20E  81FEF108          cmp si,0x8f1
0000A212  72EE              jc loc_0A202   ; ->0xA202
loc_0A214:
0000A214  B800E0            mov ax,0xe000
0000A217  E80100            call sub_0A21B   ; ->0xA21B
loc_0A21A:
0000A21A  C3                ret
sub_0A21B:
...
0000A22B  740B              jz loc_0A238   ; ->0xA238
0000A22D  83C712            add di,byte +0x12
0000A230  81FF2001          cmp di,0x120
0000A234  72E9              jc loc_0A21F   ; ->0xA21F
...
0000A244  7217              jc loc_0A25D   ; ->0xA25D
0000A246  81FBCA18          cmp bx,0x18ca
0000A24A  7311              jnc loc_0A25D   ; ->0xA25D
0000A24C  A11409            mov ax,[0x914]
0000A24F  2B4708            sub ax,[bx+0x8]
0000A252  7509              jnz loc_0A25D   ; ->0xA25D
...
0000A273  72E8              jc loc_0A25D   ; ->0xA25D
0000A275  81FBCA18          cmp bx,0x18ca
0000A279  73E2              jnc loc_0A25D   ; ->0xA25D
0000A27B  A11409            mov ax,[0x914]
0000A27E  3B4708            cmp ax,[bx+0x8]
0000A281  75DA              jnz loc_0A25D   ; ->0xA25D
0000A283  58                pop ax
0000A284  50                push ax
0000A285  51                push cx
0000A286  52                push dx
...
0000A29C  7506              jnz loc_0A2A4   ; ->0xA2A4
0000A29E  3BC1              cmp ax,cx
0000A2A0  7302              jnc loc_0A2A4   ; ->0xA2A4
0000A2A2  8BC8              mov cx,ax
loc_0A2A4:
0000A2A4  E337              jcxz loc_0A2DD   ; ->0xA2DD
...
loc_0A2E7:
0000A2E7  E973FF            jmp loc_0A25D   ; ->0xA25D
0000A2EA  5B                pop bx
0000A2EB  83FB05            cmp bx,byte +0x5
0000A2EE  73F7              jnc loc_0A2E7   ; ->0xA2E7
0000A2F0  56                push si
0000A2F1  51                push cx
...
0000A312  72D0              jc loc_0A2E4   ; ->0xA2E4
0000A314  81FBCA18          cmp bx,0x18ca
0000A318  73CA              jnc loc_0A2E4   ; ->0xA2E4
0000A31A  A11409            mov ax,[0x914]
0000A31D  3B4708            cmp ax,[bx+0x8]
0000A320  75C2              jnz loc_0A2E4   ; ->0xA2E4
0000A322  58                pop ax
0000A323  06                push es
0000A324  57                push di
0000A325  51                push cx
...
0000A32A  7208              jc loc_0A334   ; ->0xA334
0000A32C  7729              ja loc_0A357   ; ->0xA357
0000A32E  035704            add dx,[bx+0x4]
0000A331  134F06            adc cx,[bx+0x6]
loc_0A334:
0000A334  263B4D0E          cmp cx,[es:di+0xe]
0000A338  7731              ja loc_0A36B   ; ->0xA36B
0000A33A  7206              jc loc_0A342   ; ->0xA342
0000A33C  263B550C          cmp dx,[es:di+0xc]
0000A340  7729              ja loc_0A36B   ; ->0xA36B
loc_0A342:
...
0000A363  73E1              jnc loc_0A346   ; ->0xA346
...
0000A377  7502              jnz loc_0A37B   ; ->0xA37B
...
0000A392  7312              jnc loc_0A3A6   ; ->0xA3A6
;>>>> [INT4F-dispatch] Sets the return code DX=0x2 in a range/table dispatcher (after 'cmp bx,0x164a'/'cmp bx,0x18ca' classifies the index BX); falls into the shared exit 'pop ax / clc / retf 2' returning success with DX=0x02 as the selected handler/parameter value, parallel to the DX=0x80 case at 0xA3A1.  // BA0200 mov dx,0x2 after cmp bx,0x18ca/jnc, before pop ax/clc/retf 2; sibling mov dx,0x80 at 0xA3A1
...
0000A39F  7305              jnc loc_0A3A6   ; ->0xA3A6
...
0000A3B4  1E                push ds
0000A3B5  06                push es
0000A3B6  E8FFFD            call sub_0A1B8   ; ->0xA1B8
0000A3B9  E83CFE            call sub_0A1F8   ; ->0xA1F8
0000A3BC  7537              jnz loc_0A3F5   ; ->0xA3F5
0000A3BE  06                push es
...
0000A3E1  7603              jna loc_0A3E6   ; ->0xA3E6
0000A3E3  AA                stosb
0000A3E4  E2F8              loop loc_0A3DE   ; ->0xA3DE
loc_0A3E6:
...
0000A493  5E                pop si
0000A494  56                push si
;>>>> [INT21-DOS] Memory-allocation/free-list walk (called from heap manager): after pushing the current block pointer SI, call 0xA581 tests/processes a heap block descriptor; JNZ back to 0xA490 on mismatch, else advance SI by 8 (descriptor size) and loop until si reaches 0x8F1. Part of the INT 21h AH=48/4A-style allocator.  // 'call 0xa581 / jnz / add si,byte +0x8 / cmp si,0x8f1 / jc' descriptor-table walk; results stored to [0x18d7]/[0x18d9]
0000A495  E8E900            call dos_blk_desc_0A581   ; ->0xA581
0000A498  75F6              jnz dos_blk_desc_0A490   ; ->0xA490
loc_0A49A:
0000A49A  5E                pop si
0000A49B  83C608            add si,byte +0x8
0000A49E  81FEF108          cmp si,0x8f1
0000A4A2  72C8              jc loc_0A46C   ; ->0xA46C
...
0000A4BA  732A              jnc loc_0A4E6   ; ->0xA4E6
...
0000A501  E86000            call video_blk_fill_0A564   ; ->0xA564
0000A504  5E                pop si
0000A505  7559              jnz loc_0A560   ; ->0xA560
0000A507  A1DB18            mov ax,[0x18db]
0000A50A  0304              add ax,[si]
0000A50C  8EC0              mov es,ax
;>>>> [INT21-DOS/memory-alloc] XOR DI,DI zeroes the destination offset before the heap block-descriptor copy/process: ES was just set to the block segment ([0x18db]+[si]) and the routine then sets up DS:SI from [bp-8] and calls 0xa564 with CX=[bp-0xc] (the 0x12-stride descriptor processor). Part of the INT 21h AH=4A-style memory-block resize/merge in the CONFIG.BLK heap.  // 'xor di,di' after 'mov es,ax' (ax=[0x18db]+[si]); followed by 'lds si,[bp-0x8]' and 'call 0xa564' with cx=[bp-0xc]
0000A50E  33FF              xor di,di
0000A510  C576F8            lds si,[bp-0x8]
0000A513  837EF600          cmp word [bp-0xa],byte +0x0
0000A517  751C              jnz loc_0A535   ; ->0xA535
0000A519  817EF40080        cmp word [bp-0xc],0x8000
0000A51E  7715              ja loc_0A535   ; ->0xA535
0000A520  8B4EF4            mov cx,[bp-0xc]
0000A523  E83E00            call video_blk_fill_0A564   ; ->0xA564
0000A526  7534              jnz loc_0A55C   ; ->0xA55C
...
0000A538  E82900            call video_blk_fill_0A564   ; ->0xA564
0000A53B  751F              jnz loc_0A55C   ; ->0xA55C
...
video_blk_fill_0A564:
0000A564  B050              mov al,0x50
0000A566  AA                stosb
0000A567  4F                dec di
blk_desc_0A568:
;>>>> [INT21-DOS/memory-alloc] Emits marker/length byte 0x40 (via the following stosb) into the heap-block descriptor name/string buffer being built; this is one case of a small switch (sibling cases write 0x50, 0x3, 0x4) feeding a lodsb/scan-for-high-bit (test al,0x80) terminator loop that finalizes the descriptor entry.  // mov al,0x50/stosb (prev) ; >>mov al,0x40 / stosb / lodsb / dec di / test al,0x80 / loop 0xa568
...
0000A574  74F8              jz loc_0A56E   ; ->0xA56E
0000A576  E2F0              loop blk_desc_0A568   ; ->0xA568
...
0000A588  7603              jna loc_0A58D   ; ->0xA58D
...
0000A599  721C              jc loc_0A5B7   ; ->0xA5B7
...
0000A5DC  74F9              jz loc_0A5D7   ; ->0xA5D7
...
0000A5EA  7406              jz loc_0A5F2   ; ->0xA5F2
...
0000A611  747F              jz loc_0A692   ; ->0xA692
0000A613  3CFF              cmp al,0xff
0000A615  747B              jz loc_0A692   ; ->0xA692
...
;>>>> [INT21-DOS/memory-alloc] Part of a 32-bit logical >>4 (divide-by-16) converting a byte length in CX:AX (already rounded up by +0xF at 0xA620/0xA623) into a paragraph count: 'ror cx,4' rotates the high word, then 'shr ax,4' and 'or ax,cx' merge to form the paragraph size; feeds the heap/free-list block sizing that updates the descriptor's available field [es:bx+6]/[es:bx+4].  // C1C904 ror cx,4 following add ax,0xf / adc cx,0; then shr ax,4 / or ax,cx; sub [es:bx+0x6],ax
0000A626  C1C904            ror cx,byte 0x4
0000A629  C1E804            shr ax,byte 0x4
;>>>> [INT21-DOS] Combines the high word (CX, ror'd) and low word (AX, shr 4) of a (length+0xF)>>4 computation into a paragraph (16-byte) count, used to bump the block descriptor fields [es:bx+4]/[es:bx+6] in the memory allocator/arena manager.  // 0x0A620 add ax,0xf / adc cx,0 / ror cx,4 / shr ax,4 then 0x0A62C 0BC1 'or ax,cx'; result subtracted/added to [es:bx+6]/[es:bx+4]
...
;>>>> [INT21-DOS/memory-alloc] Calls the heap/block descriptor processor (0xa564) with CX=0x12 (18-byte block-descriptor stride) after pushing es/di/ds/si/ax and the running size (cx+=es) — part of the memory-arena walk that strides 18-byte descriptors (same 0x12 stride/0xa564 callee as accepted 0xa697).  // mov cx,0x12 / call 0xa564 / cmp cx,0x800
0000A64E  E813FF            call video_blk_fill_0A564   ; ->0xA564
0000A651  07                pop es
0000A652  59                pop cx
...
0000A688  E8D9FE            call video_blk_fill_0A564   ; ->0xA564
0000A68B  5E                pop si
0000A68C  1F                pop ds
;>>>> [INT21-DOS/memory-alloc] Pops saved DI as part of the epilogue of the memory-block scan routine, restoring registers (si,ds,di,es) that were pushed before the block-walk call 0xa564.  // pop di in sequence 'pop si / pop ds / pop di / pop es' after call 0xa564
0000A68D  5F                pop di
0000A68E  07                pop es
0000A68F  83EF12            sub di,byte +0x12
loc_0A692:
0000A692  83EE12            sub si,byte +0x12
0000A695  73E8              jnc blk_desc_0A67F   ; ->0xA67F
;>>>> [INT21-DOS/memory-alloc] Restores the caller's DS segment (from DX) at the exit of a memory-block walk routine that strides through 18-byte (0x12) block descriptors via call 0xa564, before popping BP and returning AL=0 (fail).  // mov ds,dx after loop 'sub si,0x12 / sub di,0x12 / jnc 0xa67f'; followed by pop bp / xor al,al / ret
0000A697  8EDA              mov ds,dx
0000A699  5D                pop bp
0000A69A  32C0              xor al,al
0000A69C  C3                ret
sub_0A69D:
0000A69D  E818FB            call sub_0A1B8   ; ->0xA1B8
0000A6A0  E855FB            call sub_0A1F8   ; ->0xA1F8
0000A6A3  750A              jnz loc_0A6AF   ; ->0xA6AF
...
0000A6D1  E81200            call sub_0A6E6   ; ->0xA6E6
...
0000A6F8  740C              jz loc_0A706   ; ->0xA706
...
0000A714  7405              jz loc_0A71B   ; ->0xA71B
0000A716  AA                stosb
0000A717  E2F8              loop loc_0A711   ; ->0xA711
...
0000A79C  E8E5F6            call sub_09E84   ; ->0x9E84
...
comm_cursor_blk_0A82A:
;>>>> [INT40-comm] PUSH BX at the entry of a page/context-switch handler that activates a new screen/page control block: it saves the old active-page pointer [0x978] into [0x918], stores the new page AX into [0x978] and [0x914], then loads the saved cursor row/col (ch/cl=[bx+0x9c/0x9d]) and attributes (dh/dl=[bx+0x9e/0x9f]) to restore that page's display state. Immediately follows the prior task's far-return (retf 2 at 0xa827).  // 'push bx' then 'mov bx,[0x978]'/'mov [0x918],bx'/'mov [0x978],ax'/'mov [0x914],ax'; loads [bx+0x9c..0x9f]
0000A82A  53                push bx
...
;>>>> [INT40-comm] In an INT handler (DS=0x40): tests the DX parameter for zero; if nonzero and within the control-block range 0x18de..0x245e it stores DX into state word [0x7fe], validating an active screen/page control-block pointer before fetching [0x918]->[bx+0xa9]. The 0x245e upper bound matches the CB-table limit used at 0x6e9a.  // >>test dx,dx / jz 0xa889 / cmp dx,0x18de / jc / cmp dx,0x245e / jnc / mov [0x7fe],dx
0000A875  85D2              test dx,dx
0000A877  7410              jz comm_blk_fhandle_0A889   ; ->0xA889
0000A879  81FADE18          cmp dx,0x18de
0000A87D  7216              jc loc_0A895   ; ->0xA895
0000A87F  81FA5E24          cmp dx,0x245e
0000A883  7304              jnc comm_blk_fhandle_0A889   ; ->0xA889
0000A885  8916FE07          mov [0x7fe],dx
comm_blk_fhandle_0A889:
...
0000A89D  7406              jz loc_0A8A5   ; ->0xA8A5
0000A89F  3A97A900          cmp dl,[bx+0xa9]
0000A8A3  740C              jz loc_0A8B1   ; ->0xA8B1
loc_0A8A5:
0000A8A5  81C3B800          add bx,0xb8
0000A8A9  81FB5E24          cmp bx,0x245e
0000A8AD  72E9              jc loc_0A898   ; ->0xA898
...
0000A8E2  7409              jz loc_0A8ED   ; ->0xA8ED
...
0000A96E  7505              jnz loc_0A975   ; ->0xA975
0000A970  3B4404            cmp ax,[si+0x4]
0000A973  740B              jz loc_0A980   ; ->0xA980
loc_0A975:
0000A975  81C6B800          add si,0xb8
0000A979  81FE5E24          cmp si,0x245e
0000A97D  72EA              jc loc_0A969   ; ->0xA969
0000A97F  C3                ret
loc_0A980:
...
0000AA03  7516              jnz loc_0AA1B   ; ->0xAA1B
loc_0AA05:
0000AA05  53                push bx
0000AA06  E86C00            call sub_0AA75   ; ->0xAA75
0000AA09  5B                pop bx
...
0000AA34  7408              jz loc_0AA3E   ; ->0xAA3E
0000AA36  3A06B507          cmp al,[0x7b5]
0000AA3A  750C              jnz loc_0AA48   ; ->0xAA48
0000AA3C  32C0              xor al,al
loc_0AA3E:
0000AA3E  A25009            mov [0x950],al
0000AA41  53                push bx
0000AA42  E82600            call cursor_fread_0AA6B   ; ->0xAA6B
0000AA45  5B                pop bx
...
0000AA54  7203              jc loc_0AA59   ; ->0xAA59
0000AA56  BE5E25            mov si,0x255e
loc_0AA59:
0000AA59  3B364809          cmp si,[0x948]
0000AA5D  7404              jz loc_0AA63   ; ->0xAA63
0000AA5F  89364609          mov [0x946],si
loc_0AA63:
0000AA63  58                pop ax
0000AA64  A92000            test ax,0x20
0000AA67  759C              jnz loc_0AA05   ; ->0xAA05
0000AA69  EB9F              jmp short 0xaa0a
cursor_fread_0AA6B:
0000AA6B  FA                cli
;>>>> [timer/ICU] Re-entrancy/busy guard at entry of a CLI-protected routine: after disabling interrupts it loads flag byte [0x94f] and 'test al,al; jz' — if the flag is nonzero it re-enables interrupts (STI at 0xaa73) and returns immediately, otherwise proceeds. Reads the lock/active flag for the cursor/scroll state machine that also tracks [0x946]/[0x948].  // cli at 0xaa6b; mov al,[0x94f]; test al,al; jz 0xaa76; sti; ret guard pattern
0000AA6C  A04F09            mov al,[0x94f]
0000AA6F  84C0              test al,al
0000AA71  7403              jz cursor_fread_0AA76   ; ->0xAA76
0000AA73  FB                sti
0000AA74  C3                ret
sub_0AA75:
0000AA75  FA                cli
...
0000AA7C  755F              jnz loc_0AADD   ; ->0xAADD
0000AA7E  A0A807            mov al,[0x7a8]
0000AA81  3C02              cmp al,0x2
0000AA83  7404              jz loc_0AA89   ; ->0xAA89
...
0000AAEF  75F5              jnz loc_0AAE6   ; ->0xAAE6
0000AAF1  C3                ret
0000AAF2  1E                push ds
...
0000AB43  7502              jnz loc_0AB47   ; ->0xAB47
...
0000AB4E  74B8              jz loc_0AB08   ; ->0xAB08
0000AB50  3C02              cmp al,0x2
0000AB52  74B4              jz loc_0AB08   ; ->0xAB08
...
0000ABD6  73F2              jnc loc_0ABCA   ; ->0xABCA
...
;>>>> [string/data] Data misdecoded as code: this 0x4B byte is part of a little-endian word/offset table (0x4BDD,0x4BDD,0x4BDF,0x4BE4,0x4BF0) sitting between the 'jmp short' dispatch at 0xABEE/0xABF0 and the resumed handler at 0xABFC; ndisasm renders the DD/DF/4B/E4/F0 bytes as bogus fisttp/dec/lock instructions.  // bytes DD4B DD4B DF4B E4 4B F04B form ascending 0x4Bxx words; real code (push dx) resumes at 0xABFC
0000ABF9  4B                dec bx
0000ABFA  F04B              lock dec bx
0000ABFC  52                push dx
0000ABFD  803EA80702        cmp byte [0x7a8],0x2
0000AC02  7407              jz loc_0AC0B   ; ->0xAC0B
0000AC04  803EA80701        cmp byte [0x7a8],0x1
0000AC09  75BF              jnz loc_0ABCA   ; ->0xABCA
loc_0AC0B:
...
0000AC30  7509              jnz comm_fhandle_0AC3B   ; ->0xAC3B
0000AC32  0E                push cs
0000AC33  E837FF            call comm_fhandle_0AB6D   ; ->0xAB6D
0000AC36  1F                pop ds
0000AC37  5B                pop bx
...
;>>>> [INT40-comm] Pushes CS so the following near 'call 0xab6d' returns into this segment and the handler can exit via 'retf 2': this is an INT 40h sub-handler that first zeroes terminal state byte [0x7a8] (with DS=0x0400) then invokes the shared worker 0xab6d before the far-return-2 epilogue.  // push cs; call 0xab6d; pop ds; pop bx; retf 0x2 (matching the parallel handler at 0xac3b/0xac52)
0000AC49  0E                push cs
0000AC4A  E820FF            call comm_fhandle_0AB6D   ; ->0xAB6D
0000AC4D  1F                pop ds
0000AC4E  5B                pop bx
...
0000AC5D  0E                push cs
0000AC5E  E80500            call sub_0AC66   ; ->0xAC66
0000AC61  1F                pop ds
0000AC62  5B                pop bx
...
0000AC6B  E93FFF            jmp loc_0ABAD   ; ->0xABAD
0000AC6E  50                push ax
0000AC6F  1E                push ds
0000AC70  51                push cx
0000AC71  52                push dx
...
0000AC8A  761F              jna loc_0ACAB   ; ->0xACAB
...
0000ACA0  740E              jz loc_0ACB0   ; ->0xACB0
...
;>>>> [timer] CLI guards atomic xchg of countdown word [0x964]; the FF30/FF32/FF34/FF36 OUTs reprogram the 80C188EB Interrupt Control Unit (FF36=mask), not the timer count registers.  // cli; xchg ax,[0x964]; test ax,ax; cmp ax,[0x966]; sti; out to dx=0xff32 / dx=0xff34
0000ACBD  FA                cli
0000ACBE  87066409          xchg ax,[0x964]
0000ACC2  85C0              test ax,ax
0000ACC4  7508              jnz loc_0ACCE   ; ->0xACCE
0000ACC6  3B066609          cmp ax,[0x966]
0000ACCA  7519              jnz loc_0ACE5   ; ->0xACE5
0000ACCC  FB                sti
0000ACCD  C3                ret
loc_0ACCE:
0000ACCE  FB                sti
...
0000ADB1  7164              jno loc_0AE17   ; ->0xAE17
0000ADB3  50                push ax
0000ADB4  47                inc di
0000ADB5  2F                das
0000ADB6  AB                stosw
...
0000AE19  7000              jo loc_0AE1B   ; ->0xAE1B
loc_0AE1B:
0000AE1B  AE                scasb
...
;>>>> [string/data] Data misdecoded as code: bytes in the character-generator font/glyph bitmap data block of upper ROM (the run continues into the labeled font region at 0x0ae4c); 'mov ch,0xb4' / 'mov [bx+0x2021],al' are bitmap bytes, not real instructions.  // outsb/outsw/xor/and noise preceding; B5B4 88872120 B3B2 ...; adjacent labeled font block 0x0ae4c-0x0ae62 'character-generator font / bitmap data'
...
;>>>> [string/data] Data misdecoded as code: this 0x1C76 sits in the character-generator font/bitmap region (the next bytes at 0xae4c are explicitly labeled font/bitmap data, followed by zero padding at 0xae62). The surrounding xchg/db garbage shows ndisasm is decoding glyph bitmap bytes, not 'sbb al,0x76'.  // immediately followed by labeled '0x0ae4c-0x0ae62 character-generator font / bitmap data' and zero padding block
...
0000AE9C  9D                popf
0000AE9D  9C                pushf
;>>>> [string/data] Data misdecoded as code: bytes inside the character-generator font/glyph bitmap region of upper ROM; ndisasm renders the glyph byte stream as a bogus 'wait call 0x5063:0xa599' far call. Not executable code (the immediately following bytes 0x0AE4C are labeled font/bitmap data in the annotated disasm).  // 9B9A99A56350 -> 'wait call 0x5063:0xa599'; adjacent labeled font block at 0x0ae4c-0x0ae62; surrounding popf/pushf/daa noise
...
;>>>> [timer/ICU] STOSW writes CS as the segment half of an interrupt vector during IVT setup (under CLI). DI=0x20 targets IVT entry 0x08 (the 80C188EB timer/ICU interrupt); the offset 0x5207 was stored just before, and this stores AX=CS. Surrounding code likewise installs vector 0x70/INT 0x1C (off 0x5211) and 0x4C/INT 0x13 (off 0x4FEE), then programs PCB regs FF42/FF46.  // 'mov ax,cs' / STOSW (AB) to ES:DI, DI=0x20 (INT 0x08); under CLI; followed by OUT to 0xff42/0xff46
0000AF1D  AB                stosw
...
;>>>> [timer] Computes a timing/blink-rate reload value: AL=5 is multiplied by the cursor/refresh divisor byte [0x7b7] and the 16-bit product's low byte stored at [0x970]; this seeds the cursor-blink or tick counter that the timer ISR decrements (companion to the descending baud/parameter index ladder that follows at 0xaf5d-0xaf8f).  // mov al,0x5 / mul byte [0x7b7] / mov [0x970],al
...
0000AF5F  7450              jz scc_baud_0AFB1   ; ->0xAFB1
0000AF61  B437              mov ah,0x37
0000AF63  3C01              cmp al,0x1
0000AF65  764A              jna scc_baud_0AFB1   ; ->0xAFB1
0000AF67  B41C              mov ah,0x1c
0000AF69  3C02              cmp al,0x2
0000AF6B  7644              jna scc_baud_0AFB1   ; ->0xAFB1
0000AF6D  B412              mov ah,0x12
0000AF6F  3C03              cmp al,0x3
0000AF71  763E              jna scc_baud_0AFB1   ; ->0xAFB1
0000AF73  B40E              mov ah,0xe
0000AF75  3C04              cmp al,0x4
0000AF77  7638              jna scc_baud_0AFB1   ; ->0xAFB1
0000AF79  B40B              mov ah,0xb
0000AF7B  3C05              cmp al,0x5
0000AF7D  7632              jna scc_baud_0AFB1   ; ->0xAFB1
0000AF7F  B409              mov ah,0x9
0000AF81  3C06              cmp al,0x6
0000AF83  762C              jna scc_baud_0AFB1   ; ->0xAFB1
0000AF85  B408              mov ah,0x8
0000AF87  3C07              cmp al,0x7
0000AF89  7626              jna scc_baud_0AFB1   ; ->0xAFB1
0000AF8B  B407              mov ah,0x7
0000AF8D  3C08              cmp al,0x8
;>>>> [SCC/serial] Branch in a descending threshold ladder mapping an index in AL to a code value in AH (table 5,6,7,8,9,0xc,0x10,0x17...); jna to common exit 0xafb1 when AL<=8 selects code AH=7, a baud-rate/parameter index lookup.  // jna 0xafb1 within repeated 'mov ah,N / cmp al,K / jna 0xafb1' pattern
0000AF8F  7620              jna scc_baud_0AFB1   ; ->0xAFB1
0000AF91  B406              mov ah,0x6
0000AF93  3C09              cmp al,0x9
0000AF95  761A              jna scc_baud_0AFB1   ; ->0xAFB1
0000AF97  B405              mov ah,0x5
0000AF99  3C0C              cmp al,0xc
0000AF9B  7614              jna scc_baud_0AFB1   ; ->0xAFB1
0000AF9D  B404              mov ah,0x4
0000AF9F  3C10              cmp al,0x10
0000AFA1  760E              jna scc_baud_0AFB1   ; ->0xAFB1
0000AFA3  B403              mov ah,0x3
0000AFA5  3C17              cmp al,0x17
0000AFA7  7608              jna scc_baud_0AFB1   ; ->0xAFB1
0000AFA9  B402              mov ah,0x2
0000AFAB  3C2D              cmp al,0x2d
0000AFAD  7602              jna scc_baud_0AFB1   ; ->0xAFB1
...
loc_0AFCC:
0000AFCC  E90701            jmp loc_0B0D6   ; ->0xB0D6
0000AFCF  3C12              cmp al,0x12
0000AFD1  7414              jz loc_0AFE7   ; ->0xAFE7
0000AFD3  3C13              cmp al,0x13
0000AFD5  7410              jz loc_0AFE7   ; ->0xAFE7
0000AFD7  3C41              cmp al,0x41
0000AFD9  740C              jz loc_0AFE7   ; ->0xAFE7
0000AFDB  3CC1              cmp al,0xc1
0000AFDD  7408              jz loc_0AFE7   ; ->0xAFE7
0000AFDF  3C06              cmp al,0x6
0000AFE1  7404              jz loc_0AFE7   ; ->0xAFE7
0000AFE3  3C10              cmp al,0x10
0000AFE5  75CF              jnz loc_0AFB6   ; ->0xAFB6
loc_0AFE7:
...
0000B019  75B1              jnz loc_0AFCC   ; ->0xAFCC
0000B01B  06                push es
0000B01C  56                push si
0000B01D  57                push di
0000B01E  51                push cx
0000B01F  B91000            mov cx,0x10
0000B022  E8B301            call timer_fread_clear_0B1D8   ; ->0xB1D8
0000B025  A35E28            mov [0x285e],ax
0000B028  B110              mov cl,0x10
0000B02A  E8AB01            call timer_fread_clear_0B1D8   ; ->0xB1D8
0000B02D  A36028            mov [0x2860],ax
0000B030  B110              mov cl,0x10
0000B032  E8A301            call timer_fread_clear_0B1D8   ; ->0xB1D8
0000B035  A36228            mov [0x2862],ax
0000B038  B110              mov cl,0x10
0000B03A  E89B01            call timer_fread_clear_0B1D8   ; ->0xB1D8
0000B03D  A36428            mov [0x2864],ax
0000B040  B110              mov cl,0x10
0000B042  E89301            call timer_fread_clear_0B1D8   ; ->0xB1D8
0000B045  A36628            mov [0x2866],ax
0000B048  B110              mov cl,0x10
0000B04A  E88B01            call timer_fread_clear_0B1D8   ; ->0xB1D8
0000B04D  A36828            mov [0x2868],ax
0000B050  B110              mov cl,0x10
0000B052  E88301            call timer_fread_clear_0B1D8   ; ->0xB1D8
0000B055  A36A28            mov [0x286a],ax
0000B058  B108              mov cl,0x8
0000B05A  E87B01            call timer_fread_clear_0B1D8   ; ->0xB1D8
0000B05D  A26C28            mov [0x286c],al
0000B060  B120              mov cl,0x20
0000B062  E87301            call timer_fread_clear_0B1D8   ; ->0xB1D8
0000B065  3A06F407          cmp al,[0x7f4]
0000B069  7581              jnz loc_0AFEC   ; ->0xAFEC
0000B06B  8B1E7809          mov bx,[0x978]
0000B06F  8A879700          mov al,[bx+0x97]
0000B073  84C0              test al,al
0000B075  7503              jnz loc_0B07A   ; ->0xB07A
0000B077  A0F707            mov al,[0x7f7]
loc_0B07A:
0000B07A  803EF40710        cmp byte [0x7f4],0x10
0000B07F  7433              jz loc_0B0B4   ; ->0xB0B4
0000B081  803EF40706        cmp byte [0x7f4],0x6
0000B086  742C              jz loc_0B0B4   ; ->0xB0B4
0000B088  B105              mov cl,0x5
0000B08A  BFF74E            mov di,0x4ef7
0000B08D  B420              mov ah,0x20
0000B08F  803EF40713        cmp byte [0x7f4],0x13
0000B094  740C              jz loc_0B0A2   ; ->0xB0A2
0000B096  803EF40712        cmp byte [0x7f4],0x12
0000B09B  7405              jz loc_0B0A2   ; ->0xB0A2
0000B09D  BFF24E            mov di,0x4ef2
0000B0A0  B408              mov ah,0x8
loc_0B0A2:
0000B0A2  D0E8              shr al,1
0000B0A4  7303              jnc loc_0B0A9   ; ->0xB0A9
0000B0A6  2E2A25            sub ah,[cs:di]
loc_0B0A9:
0000B0A9  47                inc di
0000B0AA  E2F6              loop loc_0B0A2   ; ->0xB0A2
0000B0AC  8ACC              mov cl,ah
0000B0AE  49                dec cx
0000B0AF  7403              jz loc_0B0B4   ; ->0xB0B4
;>>>> [timer] In the baud-rate / timer-reload setup path: after computing a shift/divisor count in CL (loop building AH from a [cs:di] subtraction table), call 0xB1D8 computes the timer reload value; the result is later programmed into PCB timer FF5E (next instructions read/modify FF5E to gate the baud/tick timer).  // call 0xb1d8 immediately followed by 'mov dx,0xff5e / cli / in al,dx / and al,0xfd / out dx,al' (PCB timer-control bit clear)
0000B0B1  E82401            call timer_fread_clear_0B1D8   ; ->0xB1D8
loc_0B0B4:
...
0000B0C9  754B              jnz loc_0B116   ; ->0xB116
timer_clear_attr_0B0CB:
0000B0CB  A17409            mov ax,[0x974]
0000B0CE  84C0              test al,al
0000B0D0  7541              jnz loc_0B113   ; ->0xB113
timer_cursor_baud_0B0D2:
0000B0D2  59                pop cx
0000B0D3  5F                pop di
0000B0D4  5E                pop si
0000B0D5  07                pop es
...
0000B0DA  E8DEFB            call sub_0ACBB   ; ->0xACBB
0000B0DD  E82C02            call sub_0B30C   ; ->0xB30C
0000B0E0  E805FA            call sub_0AAE8   ; ->0xAAE8
0000B0E3  A06F09            mov al,[0x96f]
0000B0E6  FEC8              dec al
0000B0E8  7803              js loc_0B0ED   ; ->0xB0ED
0000B0EA  A26F09            mov [0x96f],al
loc_0B0ED:
0000B0ED  FE0E6E09          dec byte [0x96e]
0000B0F1  7517              jnz loc_0B10A   ; ->0xB10A
0000B0F3  FE0E6C09          dec byte [0x96c]
0000B0F7  7511              jnz loc_0B10A   ; ->0xB10A
...
0000B112  CF                iret
loc_0B113:
0000B113  E99800            jmp loc_0B1AE   ; ->0xB1AE
loc_0B116:
0000B116  51                push cx
0000B117  56                push si
...
0000B125  740F              jz loc_0B136   ; ->0xB136
0000B127  2247FF            and al,[bx-0x1]
0000B12A  740A              jz loc_0B136   ; ->0xB136
0000B12C  8AE0              mov ah,al
0000B12E  FEC8              dec al
0000B130  22C4              and al,ah
0000B132  7402              jz loc_0B136   ; ->0xB136
0000B134  0AF4              or dh,ah
loc_0B136:
0000B136  E2EA              loop loc_0B122   ; ->0xB122
0000B138  5E                pop si
0000B139  F6D6              not dh
0000B13B  8A44FF            mov al,[si-0x1]
0000B13E  3245FF            xor al,[di-0x1]
0000B141  22C6              and al,dh
0000B143  740C              jz loc_0B151   ; ->0xB151
;>>>> [keyboard] Initializes a single-bit mask AH=0x80 and bit count CL=8 to begin a bit-scan loop (test ah,al / shr ah / loop) that locates the first differing bit after the byte comparison 'xor al,[di-1] / and al,dh' at 0xB13E; part of the keyboard scancode/keycode compare routine (PUSHA-protected, returns via jmp to key-poll at 0xB0C5/0xB0CB).  // mov ah,0x80; mov cl,0x8; test ah,al; shr ah,1; loop 0xb149 forms an MSB-first bit scan over the XOR difference
0000B145  B480              mov ah,0x80
0000B147  B108              mov cl,0x8
kbd_init_0B149:
0000B149  84C4              test ah,al
0000B14B  750D              jnz loc_0B15A   ; ->0xB15A
0000B14D  D0EC              shr ah,1
0000B14F  E2F8              loop kbd_init_0B149   ; ->0xB149
loc_0B151:
0000B151  59                pop cx
0000B152  E303              jcxz loc_0B157   ; ->0xB157
0000B154  E96EFF            jmp kbd_init_0B0C5   ; ->0xB0C5
loc_0B157:
0000B157  E971FF            jmp timer_clear_attr_0B0CB   ; ->0xB0CB
loc_0B15A:
0000B15A  60                pusha
...
0000B171  7506              jnz loc_0B179   ; ->0xB179
...
0000B18C  7418              jz loc_0B1A6   ; ->0xB1A6
0000B18E  80F913            cmp cl,0x13
0000B191  7413              jz loc_0B1A6   ; ->0xB1A6
0000B193  BB024E            mov bx,0x4e02
0000B196  80F910            cmp cl,0x10
0000B199  740B              jz loc_0B1A6   ; ->0xB1A6
0000B19B  BB7A4E            mov bx,0x4e7a
0000B19E  80F906            cmp cl,0x6
0000B1A1  7403              jz loc_0B1A6   ; ->0xB1A6
0000B1A3  BB124D            mov bx,0x4d12
loc_0B1A6:
0000B1A6  2ED7              cs xlatb
0000B1A8  E8E1C4            call kbd_0768C   ; ->0x768C
;>>>> [keyboard] POPA restores all general registers (saved by the PUSHA at routine entry) after a CS:XLATB scancode/keycode translation and the call to 0x768C (key-output/enqueue), in a keyboard scancode-to-code conversion routine; it then jmp's back to the key-poll loop at 0xB14D.  // 0x61 popa; preceded by 2ED7 cs xlatb and E8E1C4 call 0x768c; followed by EB9F jmp 0xb14d
0000B1AB  61                popa
0000B1AC  EB9F              jmp short 0xb14d
loc_0B1AE:
0000B1AE  803E710900        cmp byte [0x971],0x0
0000B1B3  7420              jz timer_cursor_baud_0B1D5   ; ->0xB1D5
0000B1B5  A07209            mov al,[0x972]
0000B1B8  84E4              test ah,ah
;>>>> [timer] In the periodic timer/cursor-blink tick handler: after testing AH (the high byte of the sub-tick counter [0x972]) for nonzero, JNZ selects which threshold to compare the current tick AL against — [0x971] vs [0x970] — to decide whether the blink/baud subtick interval has elapsed before re-arming the timer via call 0x768c.  // cmp byte [0x971],0; mov al,[0x972]; test ah,ah; jnz -> cmp al,[0x971] else cmp al,[0x970]; jc 0xb1d5; mov [0x975],al; call 0x768c; jmp 0xb0d2
0000B1BA  7506              jnz loc_0B1C2   ; ->0xB1C2
0000B1BC  3A067009          cmp al,[0x970]
0000B1C0  EB04              jmp short 0xb1c6
loc_0B1C2:
0000B1C2  3A067109          cmp al,[0x971]
0000B1C6  720D              jc timer_cursor_baud_0B1D5   ; ->0xB1D5
0000B1C8  A27509            mov [0x975],al
0000B1CB  53                push bx
0000B1CC  A07309            mov al,[0x973]
0000B1CF  32E4              xor ah,ah
0000B1D1  E8B8C4            call kbd_0768C   ; ->0x768C
0000B1D4  5B                pop bx
timer_cursor_baud_0B1D5:
0000B1D5  E9FAFE            jmp timer_cursor_baud_0B0D2   ; ->0xB0D2
timer_fread_clear_0B1D8:
...
0000B1F8  E2E4              loop loc_0B1DE   ; ->0xB1DE
...
0000B2E2  E8B8F3            call sub_0A69D   ; ->0xA69D
0000B2E5  7405              jz loc_0B2EC   ; ->0xB2EC
...
0000B304  7402              jz loc_0B308   ; ->0xB308
...
0000B314  7536              jnz loc_0B34C   ; ->0xB34C
0000B316  3A067A09          cmp al,[0x97a]
0000B31A  752F              jnz loc_0B34B   ; ->0xB34B
0000B31C  FF0E7D09          dec word [0x97d]
0000B320  7529              jnz loc_0B34B   ; ->0xB34B
0000B322  FE0E7C09          dec byte [0x97c]
0000B326  751D              jnz loc_0B345   ; ->0xB345
0000B328  803EAD0700        cmp byte [0x7ad],0x0
0000B32D  7416              jz loc_0B345   ; ->0xB345
0000B32F  53                push bx
0000B330  8B1E7809          mov bx,[0x978]
;>>>> [video/M76V020] Inside the periodic display/cursor service: tests the per-page/per-line flag byte [bx+0xA4] where BX=[0x978] (the active screen-state pointer synchronized at 0xB2CC); if zero it bumps the blink/cursor counter [0x97A] and calls 0xB363, gating a cursor-blink or refresh action.  // 80BFA40000 cmp byte [bx+0xa4],0 with bx=[0x978]; inc byte [0x97a]; call 0xb363
0000B334  80BFA40000        cmp byte [bx+0xa4],0x0
0000B339  5B                pop bx
0000B33A  7509              jnz loc_0B345   ; ->0xB345
0000B33C  FE067A09          inc byte [0x97a]
0000B340  B480              mov ah,0x80
0000B342  E81E00            call video_cursor_0B363   ; ->0xB363
loc_0B345:
...
0000B35E  74EB              jz loc_0B34B   ; ->0xB34B
...
0000B38A  7512              jnz loc_0B39E   ; ->0xB39E
...
0000B3BC  7507              jnz loc_0B3C5   ; ->0xB3C5
0000B3BE  60                pusha
0000B3BF  06                push es
0000B3C0  E8C600            call sub_0B489   ; ->0xB489
0000B3C3  07                pop es
0000B3C4  61                popa
loc_0B3C5:
0000B3C5  1F                pop ds
...
0000B3D0  740A              jz loc_0B3DC   ; ->0xB3DC
0000B3D2  3C48              cmp al,0x48
0000B3D4  740E              jz loc_0B3E4   ; ->0xB3E4
0000B3D6  E89DC0            call sub_07476   ; ->0x7476
0000B3D9  1F                pop ds
0000B3DA  5B                pop bx
0000B3DB  CF                iret
loc_0B3DC:
...
0000B3F8  7329              jnc loc_0B423   ; ->0xB423
0000B3FA  3C01              cmp al,0x1
0000B3FC  7710              ja loc_0B40E   ; ->0xB40E
0000B3FE  7407              jz loc_0B407   ; ->0xB407
...
0000B411  FA                cli
0000B412  E84EFF            call video_cursor_0B363   ; ->0xB363
...
0000B43C  7548              jnz loc_0B486   ; ->0xB486
0000B43E  06                push es
0000B43F  50                push ax
0000B440  51                push cx
0000B441  56                push si
...
;>>>> [video/M76V020] Loads the cursor/line attribute shadow byte [0x992] in the VRAM render routine (ES=0xD000, BX=0xC500 attribute plane): the byte is masked (and al,0xfd) and conditionally ORed with 0x2 from the shifted AH bit, written to [es:bx] and stored back to [0x992] — building the underline/attribute plane byte for the current cell.  // mov bx,0xc500 / >>mov al,[0x992] / and al,0xfd / shr ah,1 / or al,0x2 / mov [es:bx],al / mov [0x992],al
0000B44D  A09209            mov al,[0x992]
0000B450  24FD              and al,0xfd
0000B452  D0EC              shr ah,1
0000B454  7302              jnc loc_0B458   ; ->0xB458
...
0000B471  7304              jnc loc_0B477   ; ->0xB477
...
0000B4A4  E80A00            call sub_0B4B1   ; ->0xB4B1
0000B4A7  59                pop cx
0000B4A8  1F                pop ds
0000B4A9  5B                pop bx
0000B4AA  81C70008          add di,0x800
0000B4AE  E2EC              loop loc_0B49C   ; ->0xB49C
0000B4B0  C3                ret
sub_0B4B1:
0000B4B1  1E                push ds
0000B4B2  56                push si
...
0000B4F5  7413              jz loc_0B50A   ; ->0xB50A
0000B4F7  53                push bx
0000B4F8  55                push bp
0000B4F9  52                push dx
0000B4FA  32ED              xor ch,ch
0000B4FC  3AC1              cmp al,cl
0000B4FE  7502              jnz loc_0B502   ; ->0xB502
0000B500  B501              mov ch,0x1
loc_0B502:
0000B502  8AD8              mov bl,al
0000B504  E8AAFF            call sub_0B4B1   ; ->0xB4B1
0000B507  5A                pop dx
0000B508  5D                pop bp
0000B509  5B                pop bx
loc_0B50A:
...
0000B50F  7427              jz loc_0B538   ; ->0xB538
0000B511  84C0              test al,al
0000B513  7413              jz loc_0B528   ; ->0xB528
0000B515  3AD6              cmp dl,dh
0000B517  7523              jnz loc_0B53C   ; ->0xB53C
loc_0B519:
...
0000B526  75F1              jnz loc_0B519   ; ->0xB519
loc_0B528:
0000B528  8A07              mov al,[bx]
0000B52A  43                inc bx
0000B52B  3CFF              cmp al,0xff
0000B52D  7409              jz loc_0B538   ; ->0xB538
...
0000B53B  C3                ret
loc_0B53C:
0000B53C  7317              jnc loc_0B555   ; ->0xB555
...
0000B551  75EF              jnz loc_0B542   ; ->0xB542
...
0000B563  75F0              jnz loc_0B555   ; ->0xB555
...
0000B575  7403              jz loc_0B57A   ; ->0xB57A
0000B577  1F                pop ds
0000B578  5B                pop bx
0000B579  CF                iret
loc_0B57A:
...
loc_0B589:
0000B589  51                push cx
;>>>> [video] In a screen-cell/glyph render loop targeting VRAM at segment 0xA000: saves DI (the computed VRAM destination offset, base 0xA000 + (cell&3)<<11) before reading the next character via 'es lodsb' and computing its glyph row offset. Part of the M76V020 video character-write routine.  // 'mov di,0xa000 / and ax,0x3 / shl ax,0xb / add di,ax / push di / es lodsb'; writes to VRAM A000:DI
0000B58A  57                push di
...
0000B5AE  7418              jz loc_0B5C8   ; ->0xB5C8
0000B5B0  881EB709          mov [0x9b7],bl
0000B5B4  A0C009            mov al,[0x9c0]
0000B5B7  C536BC09          lds si,[0x9bc]
0000B5BB  3B1C              cmp bx,[si]
0000B5BD  7309              jnc loc_0B5C8   ; ->0xB5C8
0000B5BF  03DB              add bx,bx
0000B5C1  3B5802            cmp bx,[bx+si+0x2]
;>>>> [INT4F-dispatch] Bounds/range check in a table-driven control-code or function dispatcher: after doubling BX (add bx,bx) and comparing BX to the per-entry limit word [bx+si+2], JC branches when the index is within range to use the indexed handler offset (added to SI at 0xB5D7).  // cmp bx,[bx+si+0x2] at 0xB5C1 then 'jc 0xb5d5'; 0xB5D7 'add si,[bx+si+0x2]' uses lds si,[cs:0x8] table
0000B5C4  720F              jc dispatch_fhandle_0B5D5   ; ->0xB5D5
0000B5C6  D1EB              shr bx,1
loc_0B5C8:
0000B5C8  2EC5360800        lds si,[cs:0x8]
0000B5CD  B00E              mov al,0xe
0000B5CF  3B1C              cmp bx,[si]
0000B5D1  7352              jnc loc_0B625   ; ->0xB625
0000B5D3  03DB              add bx,bx
dispatch_fhandle_0B5D5:
...
comm_blk_copy_0B60A:
0000B60A  E97CFF            jmp loc_0B589   ; ->0xB589
loc_0B60D:
...
0000B61D  7513              jnz loc_0B632   ; ->0xB632
...
0000B627  5F                pop di
0000B628  59                pop cx
0000B629  E2DF              loop comm_blk_copy_0B60A   ; ->0xB60A
0000B62B  5D                pop bp
0000B62C  5F                pop di
0000B62D  5E                pop si
;>>>> [INT40-comm] Interrupt/handler epilogue: 'pop dx' restores DX as part of the register teardown (pop bp/di/si/dx/ds/bx) preceding IRET, after a block-copy loop ('rep movsw' of 8 words, 'loop 0xb60a') that shifts/scrolls control-block or screen-row table entries; the IRET at 0xB631 returns from the ISR.  // 5A pop dx within pop bp/di/si/dx/ds/bx then CF iret; preceded by rep movsw + loop
0000B62E  5A                pop dx
0000B62F  1F                pop ds
0000B630  5B                pop bx
0000B631  CF                iret
loc_0B632:
0000B632  730B              jnc loc_0B63F   ; ->0xB63F
...
0000B66C  740E              jz loc_0B67C   ; ->0xB67C
...
0000B68F  75E8              jnz loc_0B679   ; ->0xB679
...
0000B69F  E8C1FC            call video_cursor_0B363   ; ->0xB363
0000B6A2  51                push cx
0000B6A3  52                push dx
0000B6A4  57                push di
0000B6A5  06                push es
...
0000B6DD  AA                stosb
0000B6DE  E2F8              loop loc_0B6D8   ; ->0xB6D8
0000B6E0  4A                dec dx
0000B6E1  75E0              jnz loc_0B6C3   ; ->0xB6C3
0000B6E3  4B                dec bx
0000B6E4  4B                dec bx
...
0000B6F1  E80801            call video_col_0B7FC   ; ->0xB7FC
0000B6F4  B000              mov al,0x0
0000B6F6  E80301            call video_col_0B7FC   ; ->0xB7FC
0000B6F9  B08A              mov al,0x8a
0000B6FB  E8FE00            call video_col_0B7FC   ; ->0xB7FC
0000B6FE  B000              mov al,0x0
0000B700  E8F900            call video_col_0B7FC   ; ->0xB7FC
0000B703  B000              mov al,0x0
0000B705  E8F400            call video_col_0B7FC   ; ->0xB7FC
0000B708  B028              mov al,0x28
0000B70A  E8EF00            call video_col_0B7FC   ; ->0xB7FC
0000B70D  B000              mov al,0x0
0000B70F  E8EA00            call video_col_0B7FC   ; ->0xB7FC
0000B712  B0C7              mov al,0xc7
;>>>> [video/M76V020] Compares the column-mode config byte [0x989] to 0x50 (80) to choose a horizontal CRTC timing parameter: within a stream of 'mov al,X / call 0xb7fc' register writes to the video gate array, AL stays 0xc7 for 132-col but is replaced with 0x7 when [0x989]==0x50 (80-column), then sent via call 0xb7fc.  // mov al,0xc7 / >>cmp byte [0x989],0x50 / jz 0xb71d / mov al,0x7 / call 0xb7fc
0000B714  803E890950        cmp byte [0x989],0x50
0000B719  7402              jz video_col_0B71D   ; ->0xB71D
0000B71B  B007              mov al,0x7
video_col_0B71D:
0000B71D  E8DC00            call video_col_0B7FC   ; ->0xB7FC
0000B720  B010              mov al,0x10
0000B722  E8D700            call video_col_0B7FC   ; ->0xB7FC
0000B725  B001              mov al,0x1
0000B727  E8D200            call video_col_0B7FC   ; ->0xB7FC
0000B72A  B0F0              mov al,0xf0
0000B72C  E8CD00            call video_col_0B7FC   ; ->0xB7FC
0000B72F  B0F0              mov al,0xf0
0000B731  E8C800            call video_col_0B7FC   ; ->0xB7FC
0000B734  803E8A0910        cmp byte [0x98a],0x10
0000B739  720C              jc loc_0B747   ; ->0xB747
0000B73B  B02D              mov al,0x2d
0000B73D  E8BC00            call video_col_0B7FC   ; ->0xB7FC
0000B740  B0AF              mov al,0xaf
0000B742  E8B700            call video_col_0B7FC   ; ->0xB7FC
0000B745  EB0A              jmp short 0xb751
loc_0B747:
0000B747  B02C              mov al,0x2c
0000B749  E8B000            call video_col_0B7FC   ; ->0xB7FC
0000B74C  B08E              mov al,0x8e
0000B74E  E8AB00            call video_col_0B7FC   ; ->0xB7FC
0000B751  803EAF0700        cmp byte [0x7af],0x0
0000B756  7507              jnz loc_0B75F   ; ->0xB75F
...
0000B77F  50                push ax
0000B780  E87900            call video_col_0B7FC   ; ->0xB7FC
0000B783  59                pop cx
0000B784  58                pop ax
0000B785  03C1              add ax,cx
0000B787  50                push ax
0000B788  E87100            call video_col_0B7FC   ; ->0xB7FC
0000B78B  8BC2              mov ax,dx
0000B78D  E86C00            call video_col_0B7FC   ; ->0xB7FC
0000B790  B0FF              mov al,0xff
0000B792  E86700            call video_col_0B7FC   ; ->0xB7FC
0000B795  58                pop ax
...
0000B7A4  E85500            call video_col_0B7FC   ; ->0xB7FC
0000B7A7  59                pop cx
0000B7A8  FEC9              dec cl
0000B7AA  8AC1              mov al,cl
0000B7AC  E84D00            call video_col_0B7FC   ; ->0xB7FC
0000B7AF  E84A00            call video_col_0B7FC   ; ->0xB7FC
0000B7B2  B000              mov al,0x0
0000B7B4  E84500            call video_col_0B7FC   ; ->0xB7FC
...
0000B7DE  FB                sti
0000B7DF  55                push bp
0000B7E0  E8A6FC            call sub_0B489   ; ->0xB489
0000B7E3  5D                pop bp
0000B7E4  32E4              xor ah,ah
0000B7E6  E87AFB            call video_cursor_0B363   ; ->0xB363
...
0000B815  7576              jnz loc_0B88D   ; ->0xB88D
0000B817  06                push es
0000B818  50                push ax
...
0000B82B  7411              jz loc_0B83E   ; ->0xB83E
0000B82D  2AC4              sub al,ah
0000B82F  3C03              cmp al,0x3
;>>>> [video/M76V020] Range/character-class check in the VRAM (ES=0xD000) character-write routine: after AL=low7bits-AH, JC branches to 0xB84C when AL<3, selecting the control-vs-printable rendering path (attribute byte build via [0x994]); part of mapping a host char/attribute pair to the dual-plane display.  // 7219 jc 0xb84c after sub al,ah / cmp al,0x3; es=0xD000 set at 0xB819
0000B831  7219              jc video_print_vram_0B84C   ; ->0xB84C
0000B833  80FC02            cmp ah,0x2
0000B836  720D              jc loc_0B845   ; ->0xB845
...
0000B87B  7802              js loc_0B87F   ; ->0xB87F
...
0000B8A0  753D              jnz loc_0B8DF   ; ->0xB8DF
...
0000B8B5  7404              jz loc_0B8BB   ; ->0xB8BB
0000B8B7  268027F7          and byte [es:bx],0xf7
loc_0B8BB:
0000B8BB  3A368809          cmp dh,[0x988]
0000B8BF  731C              jnc loc_0B8DD   ; ->0xB8DD
...
0000B904  75F3              jnz loc_0B8F9   ; ->0xB8F9
0000B906  5B                pop bx
0000B907  53                push bx
0000B908  51                push cx
0000B909  52                push dx
...
loc_0B93A:
0000B93A  E99D00            jmp loc_0B9DA   ; ->0xB9DA
loc_0B93D:
0000B93D  E9DF00            jmp loc_0BA1F   ; ->0xBA1F
0000B940  8B1E1409          mov bx,[0x914]
0000B944  3B1E7809          cmp bx,[0x978]
0000B948  75AF              jnz loc_0B8F9   ; ->0xB8F9
0000B94A  5B                pop bx
0000B94B  53                push bx
0000B94C  51                push cx
0000B94D  52                push dx
...
0000B9C4  7314              jnc loc_0B9DA   ; ->0xB9DA
...
0000BA18  75E1              jnz loc_0B9FB   ; ->0xB9FB
0000BA1A  EBBE              jmp short 0xb9da
loc_0BA1C:
0000BA1C  E951FF            jmp loc_0B970   ; ->0xB970
loc_0BA1F:
0000BA1F  803EB60900        cmp byte [0x9b6],0x0
0000BA24  75F6              jnz loc_0BA1C   ; ->0xBA1C
0000BA26  32E4              xor ah,ah
0000BA28  3ACA              cmp cl,dl
0000BA2A  7232              jc loc_0BA5E   ; ->0xBA5E
...
0000BA8D  7404              jz loc_0BA93   ; ->0xBA93
...
0000BACA  75F9              jnz loc_0BAC5   ; ->0xBAC5
0000BACC  E9E9FE            jmp loc_0B9B8   ; ->0xB9B8
0000BACF  56                push si
0000BAD0  52                push dx
0000BAD1  06                push es
0000BAD2  BE00D0            mov si,0xd000
;>>>> [video/M76V020] Sets ES=0xD000 (VRAM segment) at the entry of the character-write/output routine: SI was loaded 0xD000 then copied to ES, and the routine next fetches cursor offset [0x7ba] and attribute/state [0x9ba] before dispatching on the character in AL for control codes BS(0x08)/CR(0x0D)/BEL(0x07)/LF(0x0A) vs printable rendering into the dual-plane display.  // mov si,0xd000 then mov es,si; followed by cmp al,0x8/0xd/0x7/0xa control-char dispatch and [0x7ba] cursor load
0000BAD5  8EC6              mov es,si
0000BAD7  8B36BA07          mov si,[0x7ba]
0000BADB  8B16BA09          mov dx,[0x9ba]
0000BADF  3C08              cmp al,0x8
0000BAE1  747C              jz loc_0BB5F   ; ->0xBB5F
0000BAE3  3C0D              cmp al,0xd
0000BAE5  747B              jz loc_0BB62   ; ->0xBB62
0000BAE7  3C07              cmp al,0x7
0000BAE9  747B              jz loc_0BB66   ; ->0xBB66
0000BAEB  3C0A              cmp al,0xa
0000BAED  744C              jz loc_0BB3B   ; ->0xBB3B
0000BAEF  8B1EB809          mov bx,[0x9b8]
0000BAF3  03DA              add bx,dx
0000BAF5  8807              mov [bx],al
0000BAF7  3B367809          cmp si,[0x978]
0000BAFB  7529              jnz loc_0BB26   ; ->0xBB26
0000BAFD  803E860900        cmp byte [0x986],0x0
0000BB02  7522              jnz loc_0BB26   ; ->0xBB26
...
0000BB1E  7906              jns loc_0BB26   ; ->0xBB26
...
0000BB45  7512              jnz loc_0BB59   ; ->0xBB59
0000BB47  803E860900        cmp byte [0x986],0x0
0000BB4C  750B              jnz loc_0BB59   ; ->0xBB59
...
0000BB83  7203              jc loc_0BB88   ; ->0xBB88
...
;>>>> [video/M76V020] Fills a screen-line region of the VRAM character plane with the space character (0x20) via REP STOSB; the companion fill at 0xBBEB zeroes the attribute plane 0x8000 bytes away — clearing a line of the terminal display.  // 0x0BBDD mov al,0x20 then 0x0BBDF F3AA 'rep stosb'; later add di,0x8000 / mov al,0 / rep stosb (attribute plane)
...
;>>>> [video/M76V020] Sets attribute-plane bit 0x08 for a character cell: ES points at VRAM (0xD000) and BX holds the byteswapped cell offset just stored at [0x98b]; OR sets the attribute flag in the attribute plane.  // or byte [es:bx],0x8 after 'mov [0x98b],bx'; ES loaded 0xd000 nearby; xchg bl,bh byteswap
0000BBF8  26800F08          or byte [es:bx],0x8
loc_0BBFC:
0000BBFC  59                pop cx
0000BBFD  5F                pop di
0000BBFE  E958FF            jmp loc_0BB59   ; ->0xBB59
...
0000BC34  7203              jc loc_0BC39   ; ->0xBC39
...
0000BC51  7906              jns loc_0BC59   ; ->0xBC59
0000BC53  26C685FF7F40      mov byte [es:di+0x7fff],0x40
loc_0BC59:
0000BC59  E2ED              loop loc_0BC48   ; ->0xBC48
0000BC5B  4A                dec dx
0000BC5C  75D2              jnz loc_0BC30   ; ->0xBC30
...
0000BC76  E84C00            call video_cursor_fread_0BCC5   ; ->0xBCC5
0000BC79  47                inc di
...
0000BC93  7312              jnc loc_0BCA7   ; ->0xBCA7
0000BC95  E8FEA4            call 0x6196
0000BC98  803E850900        cmp byte [0x985],0x0
0000BC9D  740F              jz loc_0BCAE   ; ->0xBCAE
0000BC9F  C606850900        mov byte [0x985],0x0
0000BCA4  E974FF            jmp loc_0BC1B   ; ->0xBC1B
loc_0BCA7:
...
0000BCBC  E953FF            jmp loc_0BC12   ; ->0xBC12
0000BCBF  E80600            call sub_0BCC8   ; ->0xBCC8
0000BCC2  E80300            call sub_0BCC8   ; ->0xBCC8
video_cursor_fread_0BCC5:
0000BCC5  E80000            call sub_0BCC8   ; ->0xBCC8
sub_0BCC8:
0000BCC8  C1C004            rol ax,byte 0x4
0000BCCB  50                push ax
0000BCCC  240F              and al,0xf
0000BCCE  3C0A              cmp al,0xa
0000BCD0  7202              jc loc_0BCD4   ; ->0xBCD4
...
;>>>> [timer/ICU] Sets ES=DI(=0) so ES points at the IVT (segment 0); the routine then writes a far interrupt vector (offset 0x5D1F / CS) to IVT entry at offset 0x38 (INT 0Eh) via STOSW pairs, installing an interrupt handler before unmasking via ICU port 0xFF1C.  // 33FF xor di,di / 8EC7 mov es,di / BF3800 mov di,0x38 / AB stosw / BA1CFF out FF1C
...
0000BD8B  7606              jna loc_0BD93   ; ->0xBD93
0000BD8D  3A068A09          cmp al,[0x98a]
0000BD91  7240              jc loc_0BDD3   ; ->0xBDD3
loc_0BD93:
...
0000C7E5  7708              ja loc_0C7EF   ; ->0xC7EF
...
;>>>> [string/data] Data misdecoded as code: this 0x0808 ('or [bx+si],cl') is part of a run of 0x08 bytes in the character-generator font/glyph bitmap region of upper ROM (preceded by 0x77DD 'ja' and 0xF8 garbage); the uniform 0x08 byte stream is glyph/attribute bitmap data, not executable instructions.  // long run of 0808 'or [bx+si],cl' with stray F8, preceded by 77DD bytes
...
;>>>> [string/data] Character-generator font/glyph bitmap data misdecoded as code: the bytes 0x80 0x7E 0x22 0x20 are glyph scan-line rows in the upper-ROM font table, rendered by ndisasm as a bogus 'cmp byte [bp+0x22],0x20'. Not executable.  // bytes 80 7E 22 20 within labeled font/bitmap block 0x0d7f5-0x0dab6+
0000DAD4  807E2220          cmp byte [bp+0x22],0x20
0000DAD8  3C20              cmp al,0x20
;>>>> [string/data] Character-generator font/glyph bitmap data misdecoded as code: the bytes 0x20 0x22 are glyph scan-line bytes in the upper-ROM font table (0x0d7f5-0x0dab6+ region), rendered by ndisasm as a bogus 'and [bp+si],ah'. Not executable.  // bytes 20 22 amid glyph rows 80 7E 22 20 3C 20; labeled font/bitmap region [R16]
0000DADA  2022              and [bp+si],ah
0000DADC  7E00              jng data_0DADE   ; ->0xDADE
data_0DADE:
;>>>> [string/data] Character-generator font/glyph bitmap data misdecoded as code: this 0x00 0x00 pair is a blank glyph row in the 0x0d7f5-0x0dab6+ font table region (surrounding 0x40/0x7E/0x82/0x22/0x20 bytes are glyph scan-line patterns). Not an executable 'add [bx+si],al'.  // surrounding glyph bytes 80 7E 22 20 3C 20; labeled disasm marks this region as character-generator font/bitmap data [R16]
...
0000DE18  7E07              jng loc_0DE21   ; ->0xDE21
...
;>>>> [string/data] Data misdecoded as code: part of a jump/offset table of little-endian word values in the EMULATOR/font region. Surrounding 08xx 'or' decodes and the following ascending 02xx word run (0214 0222 0230 ...) show this is a pointer/dispatch table, not executable instructions.  // 08EA between runs of 08xx then a regular ascending word table 0x1402,0x4c02,0x7602,0x9202 etc.; pattern is data not code
...
;>>>> [string/data] Data misdecoded as code: this 0x0348 is one entry of an ascending little-endian word/offset table (0x033A,0x0348,0x0364,0x0372,0x0380...) in the upper-ROM EMULATOR offset/pointer table region; ndisasm renders the table words as bogus 'add' instructions, not real code.  // 034803 amid ascending word run 033A 0348 0364 0372 0380
0000DE7D  034803            add cx,[bx+si+0x3]
0000DE80  56                push si
0000DE81  036403            add sp,[si+0x3]
0000DE84  7203              jc loc_0DE89   ; ->0xDE89
...
0000DEF2  7406              jz loc_0DEFA   ; ->0xDEFA
0000DEF4  82                db 0x82
0000DEF5  06                push es
0000DEF6  90                nop
0000DEF7  06                push es
...
0000DF29  E000              loopne loc_0DF2B   ; ->0xDF2B
loc_0DF2B:
...
0000DF7E  7015              jo loc_0DF95   ; ->0xDF95
...
0000DFEC  60                pusha
;>>>> [string/data] Data misdecoded as code: this is part of a table of 16-bit little-endian word values (e.g. 0x0B6E, 0x0B7C, 0x0B8A...) in the upper-ROM data/font region, an offset or attribute lookup table, not executable instructions.  // monotonic '0B xx' word sequence (0B0C,0B1A,0B28,0B36,0B44,0B52,0B6E,0B7C...) typical of a pointer/offset table; region in upper-ROM data area
0000DFED  0B6E0B            or bp,[bp+0xb]
0000DFF0  7C0B              jl loc_0DFFD   ; ->0xDFFD
...
;>>>> [string/data] Data misdecoded as code: this 0x5F byte is part of a little-endian word/offset table in the upper-ROM data region; nearby real references like 'or [0x1409],ax' (0x1409 = the [0x914] control-block area) and the surrounding 09xx/01xx word run show this is pointer/table data, not executable instructions.  // pop di / scasb / or [0x1409],ax — ascending 09xx 01xx word run, not coherent code
0000E058  5F                pop di
0000E059  AE                scasb
...
0000E0A5  EA0FCA10A0        jmp loc_0A010:0xca0f   ; ->0xA010
...
0000E0E0  0F                db 0x0f
0000E0E1  7A0F              jpe loc_0E0F2   ; ->0xE0F2
...
0000E101  9A15A815B6        call sub_0B615:0xa815   ; ->0xB615
...
0000E150  7203              jc loc_0E155   ; ->0xE155
...
0000E19A  7000              jo loc_0E19C   ; ->0xE19C
loc_0E19C:
0000E19C  54                push sp
0000E19D  0E                push cs
...
0000E272  EA0FCA10A0        jmp loc_0A010:0xca0f   ; ->0xA010
...
0000E2AD  0F                db 0x0f
0000E2AE  7A0F              jpe loc_0E2BF   ; ->0xE2BF
...
0000E3C3  7A0F              jpe loc_0E3D4   ; ->0xE3D4
...
0000E462  7015              jo loc_0E479   ; ->0xE479
0000E464  46                inc si
...
0000E482  7218              jc loc_0E49C   ; ->0xE49C
...
;>>>> [string/data] Data misdecoded as code: part of a table of little-endian 16-bit word values (an offset/pointer or attribute lookup table) in the upper-ROM EMULATOR/font data region; surrounding bytes form ascending word entries that ndisasm renders as bogus add/sbb/ret.  // context of 'ret 0xd019', 'sbb [si],ch', 'add [bx+si+0x1a],ax' — incoherent control flow in upper-ROM data block
...
;>>>> [string/data] Data misdecoded as code: this 0x06 byte is one entry of a little-endian word/offset table in the upper-ROM EMULATOR/font data region; the surrounding run of 0x1Cxx bytes ndisasm renders as bogus 'sbb al,imm'/'push es', not executable instructions.  // sequence 06 70 1C / 7E 1C / C4 1C bytes decode as nonsensical push es/jo/jng/les
0000E51D  06                push es
0000E51E  701C              jo loc_0E53C   ; ->0xE53C
...
;>>>> [string/data] Data misdecoded as code: this 0xB2 0x16 ('mov dl,0x16') is part of a little-endian word/offset table in the upper-ROM EMULATOR/font data region; the surrounding run of bogus sbb/adc/xor/push-ss decodes (e.g. 0x1942, 0x3416, 0x13D0) shows ndisasm is rendering table entries, not executable instructions.  // B216 amid sbb [bp+si+0x16],ax / xor al,0x16 / sbb ch,al garbage
...
0000E59B  E015              loopne loc_0E5B2   ; ->0xE5B2
...
;>>>> [string/data] Data misdecoded as code: this 0x5F byte is part of a little-endian word/offset table in the upper-ROM EMULATOR data region (surrounding 0x15A8,0x160A,0x1017,0x1864,0x2100 word entries); ndisasm renders the byte stream as bogus pop di/sbb/in instructions.  // byte 5F inside run of adc/sbb/retf garbage with ascending 0x1xxx word values typical of an offset table
0000E5EF  5F                pop di
...
0000E63C  EA0FCA10A0        jmp loc_0A010:0xca0f   ; ->0xA010
...
0000E660  4C                dec sp
0000E661  17                pop ss
;>>>> [string/data] Data misdecoded as code: this 0x06 byte is part of a 16-bit word table / font region in upper ROM (surrounding bytes 0x14F2,0x14C8,0x14E4,0x14AA and 0x110F,0x1722,0x1730 are little-endian table entries, with a real instruction island at 0xE66C). Not an executed 'push es'.  // isolated 0x06 amid adc/add garbage decodes (14F2/14C8/14AA) that are word-table values, not a code path; ndisasm renders 'push es'
0000E662  06                push es
0000E663  17                pop ss
...
;>>>> [string/data] Data misdecoded as code: this is part of a table of 16-bit word values (offsets/pointers) in the font/glyph or string-pointer region of upper ROM. The surrounding bytes form a sequence of little-endian words (0x11xx, 0x12xx, 0x13xx) that ndisasm renders as bogus adc/add; not executable code.  // surrounding bytes 1110 111E 1164 11DE 12C2 12D0 ... form an ascending word table; 0F5E0F decodes as SSE divps which cannot run on an 80188
...
;>>>> [string/data] Data misdecoded as code: this 0x46 byte is part of a table of 16-bit little-endian word values (0x1562, 0x15A8, 0x11D4...) in the upper-ROM pointer/offset table; the run of adc/add and stray prefixes (lock, fcom) confirm it is data, not executable instructions.  // surrounding ascending words 1562/15a8/11d4 and bogus F011/D817 fcom decodes; offset/pointer table region
0000E694  46                inc si
...
;>>>> [string/data] Data misdecoded as code: part of a table of 16-bit little-endian word values (0x12EC, 0x147E, 0x1386...) in the upper-ROM offset/pointer table region; ndisasm renders the bytes as bogus add/adc. Not executable code.  // ascending word values 12ec/147e/1386 with stray prefixes (66, lock); surrounded by adc/add misdecodes
0000E726  05EC12            add ax,0x12ec
0000E729  7610              jna loc_0E73B   ; ->0xE73B
...
0000E7D2  721F              jc loc_0E7F3   ; ->0xE7F3
...
0000E831  E01C              loopne loc_0E84F   ; ->0xE84F
0000E833  D6                salc
...
;>>>> [string/data] Data misdecoded as code: this 0x00F6 lies immediately before the ASCII string 'FONTFILE' (bytes 0x46 0x4F 0x4E 0x54 0x46 0x49 0x4C 0x45 at 0xF2A1) in a small data record, followed by 0xFF erased fill; it is a header/data byte for the FONTFILE filename entry, not an executable 'add dh,dh'.  // xxd shows 'FONTFILE' at 0xF2A1-F2A8 right after this byte, then 0xFF fill; in al,0x5d/00F6 are part of the data blob
...
0000FFF0  EA0000E0FF        jmp loc_0FFE0:0x0   ; ->0xFFE0
